...
- **Assistente 360 separa leitura, configuracao transversal e segredo:** `CanConfigureAssistant` permite que gestores de Calendar, Meta Ads, Automation e Omnichannel consultem a configuracao e o catalogo mascarado. Alterar persona, modelo, credencial selecionada ou a matriz de todas as superficies exige `CanManageAssistantConfiguration`; criar, rotacionar, importar ou excluir segredo exige `CanManageAssistantCredentials`. As duas ultimas aceitam apenas `owner`/`platform_admin` ou RBAC account-scoped resolvida com `automation.manage`, `core.account.manage` ou `workspace.configuracoes.edit`. Gestor restrito a um modulo nao reconfigura os demais dominios; permissoes nao resolvidas falham fechado.
- **Organizacao NAO preenche `TenantID` nem concede account cliente (decisao de seguranca).** Quando `account_users` nao mapeia papel-coarse, `resolveCoreAuthRoleScope` devolve escopo VAZIO (login segue). NAO derivamos um `TenantID` da organizacao porque varias rotas legadas tenant-scoped tratam `principal.TenantID` como PROVA de acesso. `account_checker` e `/v2/me/accounts` exigem membership explicita para nao-platform-admin; a conta-agencia tambem precisa estar em `core.account_users`, como ja ocorre no vinculo de organizacao.
...
- `middleware.go`
- `assistant_access.go` — politica RBAC transversal para configurar o Assistente 360 apos `RequireAuthWithAccount`
- `http.go`
