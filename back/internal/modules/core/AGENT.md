...
- `rbac_model.go` — struct `Role` com `ToSummary()`, erros RBAC em `errors.go`.
...
> **N+1 RESOLVIDO (OPT-2/F-22)**: `MeAccounts` agora carrega os modulos de todas as
> accounts em uma unica query batch via `loadModulesBatch` (service.go). O metodo
> detecta se o repositorio implementa `batchModuleLoader` (interface interna definida
> em service.go) e chama `ListEnabledModuleIDsForAccounts` (store_postgres.go) com
> `WHERE account_id = ANY($1::uuid[])`, agrupando em memoria. Fallback N+1 preservado
> para repositorios que so satisfazem `Repository` (ex.: fakes de teste).

## Invalidacao do PrincipalCache (AC-01)

- `RBACService` e `AdminUserService` recebem um `PrincipalCacheInvalidator` opcional (interface local `{InvalidateUser, InvalidateAll}` definida UMA vez em `rbac_service.go`, reusada pelo `admin_users_service.go`; local para nao importar `httpapi`). `module.go` `Build` liga os setters com `deps.PrincipalCache` (nil-check — `nil` quando o cache esta desligado).
- `RBACService`: `AssignRoleToUser`/`RemoveRoleFromUser`/`SetUserRoles` -> `InvalidateUser(userID)` (alimentam `core.user_role_assignments`, que resolve o papel-coarse/escopo do Principal via `auth/core_role_resolver.go`). `DeleteRole` -> `InvalidateAll()` (pode afetar N usuarios; sem indice papel->sessoes). `UpdateRolePermissions` (v2) **NAO** invalida — `core.role_permissions` alimenta a RBAC por-account resolvida a cada request (`/v2/me/context`), fora do Principal cacheado.
- `AdminUserService`: `UpdateUser`/`DeleteUser` -> `InvalidateUser(userID)` (cobre desativacao/`is_platform_admin`/soft-delete via painel admin v2). Sempre apos o sucesso no repo.
...
- `admin_users_model.go` — DTOs admin de users (`AdminUserView` = detalhe completo com `clientAccountId`/`isAgencyMember`; **`AdminUserListItem`** = projecao LEAN da LISTAGEM, OPT-4/F-26, sem `avatarPath`/`createdAt`/`updatedAt`; `AdminUserListFilter` com `AccountID`, `AdminCreateUserInput`, `AdminUpdateUserInput`, `MoveUserAccountInput`, `AccountMembershipView`) + interface `AdminUserRepository` (`ListUsers` retorna `[]AdminUserListItem`; `MoveUserAccount` e os demais retornam `AdminUserView`).
- `admin_users_repository.go` — `PostgresAdminUserRepository`: List com `accountCount`/`accountNames` agregados via LATERAL join SO quando `filter.IncludeAccounts` (default); com `includeAccounts=false` o join e omitido (projecao lean). List tambem aceita filtro `accountId` (`exists` em account_users+accounts ativos) e ambas List/Find selecionam `clientAccountIDSelect` (const: id do unico cliente ativo nao-agencia, ou '' p/ 0/>1) e `isAgencyMemberSelect` (const bool: `exists` membro de conta-agencia ativa) como ULTIMA coluna na mesma ordem nas duas queries. A LISTAGEM usa a projecao LEAN (OPT-4/F-26): o SELECT de `ListUsers` NAO seleciona `avatar_path`/`created_at`/`updated_at` e `scanAdminUserListRow` mapeia para `AdminUserListItem`. O detalhe (`FindAdminUser`) mantem o SELECT completo + `scanAdminUser` (le `clientAccountId` logo apos `accountNames` e `isAgencyMember` como ultimo campo — ordem importa). Find; Create (com hash de senha); Update; SoftDelete; GetMemberships; IsAccountMember; CountActivePlatformAdmins (safeguard). A paginacao (page/perPage, cap 100) e aplicada na query — a tela `/manage/users` busca por pagina, nao tudo de uma vez. O bloco de matricula/membership foi extraido para `admin_users_enroll_repository.go` (mesmo pacote core).
- `admin_users_enroll_repository.go` — matricula/membership do `PostgresAdminUserRepository`: `pgxExec` (interface pool OU tx), `enrollUserInAccount` (funcao livre, upsert da membership com `do update set is_active = true` que reativa em re-enroll + clona role queue.<papel> + perms), `agencyAccountRole`, `SetUserAccountRole` e **MoveUserAccount** (transacional: valida destino ativo + nao-agencia, remove vinculos de cliente nao-agencia, auto-enroll no destino). Separado de `admin_users_repository.go` por coesao/limite de linhas; mesmo pacote, comportamento inalterado.
