...
- `admin_users_service.go` — `AdminUserService`: valida unicidade de email (via constraint), hash de senha (`auth.BcryptHasher`) e safeguard do ultimo platform_admin antes de update/delete. `MoveUserAccount` valida `accountId` obrigatorio + papel (default `owner`, em {owner,director,marketing}) e delega ao repo transacional.
- `admin_users_http.go` — `RegisterAdminUsersRoutes`: 7 endpoints `/v1/admin/users*` (exigem platform_admin). `handleListUsers` le `accountId` da query -> `filter.AccountID`. `handleMoveUserAccount` (PUT `.../{id}/account`) move o usuario p/ a conta-cliente do body. `writeAdminUserError` mapeia `ErrAccountNotFound`->404 e `ErrAccountIsAgency`->400.
