...
func resolveCRMOrderStoreKey(explicitStoreKey string, fallbackStoreCNPJ string, employeeID string, employeeStoreFallbacks map[string]string, employeeDominantStoreKeys map[string]string) string {
	// A loja explicita do ERP e a fonte da verdade comercial. Casos especiais
	// de consultor so entram quando o pedido nao trouxe essa loja.
...
func (repository *PostgresRepository) listCRMEmployeeStoreFallbacks(ctx context.Context, tenantID string) (map[string]string, error) {
	rows, err := repository.pool.Query(ctx, `
		...
		left join queue.stores consultant_store
			on consultant_store.id = c.store_id
		   and consultant_store.tenant_id = $1::uuid
		left join lateral (
			select s.code, s.name
			from core.user_module_settings ums
			cross join lateral jsonb_array_elements_text(
				coalesce(ums.config #> array['storeIdsByAccount', $1::text], '[]'::jsonb)
			) as cfg(store_id)
			join queue.stores s
				on s.id::text = cfg.store_id
			   and s.tenant_id = $1::uuid
			where ums.user_id = u.id
			  and ums.module_id = 'queue'
		) role_store on true
