...
func buildRankingRowsAcrossBundles(bundles []bundle, scope string, dateFrom string, dateTo string) []RankingRow {
	rows := make([]RankingRow, 0)
	for _, item := range bundles {
		storeRows := buildRankingRows(item.history, item.roster, scope, dateFrom, dateTo)
...
func buildConsultantAlertsAcrossBundles(bundles []bundle, dateFrom string, dateTo string) []ConsultantAlert {
	alerts := make([]ConsultantAlert, 0)
	for _, item := range bundles {
		alerts = append(alerts, buildConsultantAlerts(item.history, item.roster, item.settings, dateFrom, dateTo)...)
	}

	return alerts
}

func buildRankingRows(history []operations.ServiceHistoryEntry, roster []operations.ConsultantProfile, scope string, dateFrom string, dateTo string) []RankingRow {
	now := time.Now().In(analyticsLocation)
	currentMonth := monthStamp(now)
	currentDay := dayStamp(now)
	rangeStart, rangeEndExclusive, hasExplicitRange := parseAnalyticsDateRange(dateFrom, dateTo)
	...
	for _, consultant := range roster {
		...
		for _, entry := range history {
			...
			case hasExplicitRange:
				if rangeStart != nil && finishedAt.Before(*rangeStart) {
					continue
				}
				if rangeEndExclusive != nil && !finishedAt.Before(*rangeEndExclusive) {
					continue
				}
			case scope == "today":
			...
			default:
				if monthStamp(finishedAt) != currentMonth {
					continue
				}
			}
...
func parseAnalyticsDateRange(dateFrom string, dateTo string) (*time.Time, *time.Time, bool) {
	var start *time.Time
	var endExclusive *time.Time

	if parsedStart, err := time.ParseInLocation("2006-01-02", strings.TrimSpace(dateFrom), analyticsLocation); err == nil {
		start = &parsedStart
	}

	if parsedEnd, err := time.ParseInLocation("2006-01-02", strings.TrimSpace(dateTo), analyticsLocation); err == nil {
		nextDay := parsedEnd.AddDate(0, 0, 1)
		endExclusive = &nextDay
	}

	return start, endExclusive, start != nil || endExclusive != nil
}
...
func buildConsultantAlerts(history []operations.ServiceHistoryEntry, roster []operations.ConsultantProfile, settings StoreSettings, dateFrom string, dateTo string) []ConsultantAlert {
	alerts := make([]ConsultantAlert, 0)
	now := time.Now().In(analyticsLocation)
	currentMonth := monthStamp(now)
	rangeStart, rangeEndExclusive, hasExplicitRange := parseAnalyticsDateRange(dateFrom, dateTo)
