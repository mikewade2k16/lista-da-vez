...
func (repository *PostgresRepository) CanAccessTenant(ctx context.Context, principal auth.Principal, tenantID string) (bool, error) {
	...
	default:
		// Membership unificada em core.account_users (a 0133 backfillou todos os
		// papeis legados, tenant E store-scoped). core.accounts.id == public.tenants.id,
		// entao au.account_id = t.id. Cobre owner/director/marketing e store-scoped.
		query = `
			select exists(
				select 1
				from core.accounts t
...
func (repository *PostgresRepository) ResolveDefaultTenantID(ctx context.Context, principal auth.Principal) (string, error) {
	...
	case auth.RolePlatformAdmin:
		...
		`
	default:
		// Tenants do usuario via membership unificada em core.account_users
		// (0133 backfillou tenant E store-scoped). Cobre owner/director/marketing
		// e store-scoped num unico caminho.
		query = `
