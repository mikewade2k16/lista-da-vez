...
} from "../data/operation-templates";
import { DEFAULT_PROFESSION_OPTIONS } from "../data/profession-options";
import { applyCampaignsToHistoryEntry, normalizeCampaign } from "../utils/campaigns";
import { cloneValue } from "../utils/object";
import { getAllowedWorkspaces } from "../utils/permissions";
import { DEFAULT_REPORT_FILTERS, normalizeReportFilters } from "../utils/reports";
...
export function createAppStore(initialState = createEmptyState()) {
  ...
  return {
    ...
    createStore({ name, city, code, cloneActiveRoster = true, defaultTemplateId = "", monthlyGoal = 0, weeklyGoal = 0, avgTicketGoal = 0, conversionGoal = 0, paGoal = 0 }) {
    ...
    createConsultantProfile({ name, role, color, monthlyGoal, commissionRate, conversionGoal = 0, avgTicketGoal = 0, paGoal = 0 }) {
      const normalizedName = String(name || "").trim();
