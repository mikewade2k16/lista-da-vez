...
ui*metadata — elas mostram o cliente como legado (badge MOCK, via `isMockClient` = não-UUID) até
...
\_Histórico do estado anterior abaixo.*
...
## 10. Página duplicada de Clientes dentro da Fila — `removido` ✅ (2026-07-25)

**O que foi removido:** os itens de navegação `Dados`, `Inteligência`,
`Relatórios` e `Clientes` saíram das abas da Fila. A UI duplicada de clientes
(`TenantsWorkspace.vue`) e as rotas `/clientes` e `/operacao/clientes` foram
excluídas. O antigo `/manage/clientes` agora redireciona para a superfície
canônica `/manage/clientes-web`.

**Auditoria do banco:** a página removida não possuía tabela própria. O fluxo
era `useTenantsStore` → `GET /v1/tenants` → `core.accounts`, exatamente a mesma
fonte usada por Clientes Web e pelos seletores de Calendar, Tasks, Bio,
Cardápio, Omnichannel e outros módulos. No banco local auditado havia 10
accounts e 249 FKs apontando para `core.accounts`; `public.tenants` já estava
ausente desde a migration 0136. Por isso nenhuma tabela e nenhum dado foram
apagados.

**Rollback:** os atalhos de `Dados`, `Inteligência` e `Relatórios` podem ser
repostos no nav sem migration; suas páginas/endpoints legados foram
preservados. A tela antiga de clientes deve ser recuperada pelo Git somente se
existir uma necessidade que a tela canônica não cubra.

---

## 11. Limpeza das abas da Config da Fila — `compatibilidade preservada` (2026-07-25)

**Removido somente da interface:** Produtos, Consultores, Metas/CRM e Alertas
deixaram de ser abas da Config. Metas/CRM foi incorporada ao bloco Metas de
Multi-loja. Motivos, Cancelamentos, Pausas, Perdas, Fora da vez, Origens e
Profissões foram reunidos na aba compacta Motivos e cadastros.

**Dados preservados:** o catálogo manual de produtos continua no banco e na API
porque `queue/catalog_adapter.go` o usa como fallback quando o ERP não entrega
catálogo. O roster/backend de consultores também continua sendo usado pela
operação. Os quatro thresholds antigos da Config continuam persistidos e
consumidos por `admin-metrics.ts`, mas não competem mais visualmente com a
central autoritativa `/alertas`.

**Gamificação mantida:** a aba não foi removida porque as regras alimentam
Ranking, Score 360, badges e perfil do consultor. A ausência de linha em
`tenant_gamification_settings` ativa defaults e não significa código morto.

**Suporte:** Feedback passou a se chamar Suporte na interface e usa `/suporte`.
IDs, permissões, tabelas e endpoints `feedback` foram mantidos, com aliases das
URLs antigas, para preservar histórico e integrações.

---

## 12. Score 360 removido da Operacao - `compatibilidade temporaria` (2026-07-25)

**Fonte funcional:** o Ranking calcula cinco componentes reais: conversao,
valor vendido, qualidade, P.A. e disciplina de fila. A configuracao visual
passou a existir somente em Gamificacao. Os templates de Operacao deixaram de
transportar e sobrescrever esses pesos; o teste de aplicacao garante que os
valores atuais sejam preservados.

**Compatibilidade mantida:** os campos `scoreWeight*` ainda vivem em
`queue.tenant_operation_core_settings` e trafegam pelo contrato operacional,
pois hoje essa e a persistencia consumida pelo bundle e pelo Ranking. Eles nao
foram apagados nesta alteracao de layout para evitar perda de configuracao ou
regressao silenciosa.

**Saida do legado:** criar destino autoritativo em Gamificacao, fazer backfill
por tenant, habilitar dual-read/dual-write, validar Ranking e somente entao
remover colunas e campos do contrato operacional. Ate esse corte, nao adicionar
criterios arbitrarios: cada novo componente precisa de fonte de dados,
normalizacao e regra de calculo reais.
