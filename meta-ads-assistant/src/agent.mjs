...
function summarizeToolCall(toolName, input) {
  const base = TOOL_SUMMARIES[toolName] || `Chamou a tool ${toolName} do MCP da Meta`;
...
function pickInputHint(input) {
  ...
  for (const key of ['name', 'campaign_name', 'entity_id', 'campaign_id', 'ad_account_id', 'account_id']) {
...
export function buildPrompt({ prompt, history, adAccountId }) {
  ...
  if (Array.isArray(history) && history.length > 0) {
    const transcript = history
      ...
      .join('\n\n');
    sections.push(`<historico_da_conversa>\n${transcript}\n</historico_da_conversa>`);
  }
...
export function buildQueryOptions(abortController, overrides = {}) {
  ...
  const options = {
    ...
    mcpServers: {
      ...
      [OMNI_SERVER_NAME]: createOmniMcpServer(),
    },
    strictMcpConfig: true,
    // Isolamento do SDK: nao carrega settings/CLAUDE.md do filesystem. A persistencia
    // do token OAuth e garantida pela SESSAO UNICA (session.mjs): uma conexao MCP
    // viva atende auth + todos os /run, entao o login fica valido pro chat inteiro.
    settingSources: [],
    // Zera o conjunto de tools built-in (Bash/Read/Web/etc nao existem na sessao).
    tools: [],
    disallowedTools: DISALLOWED_BUILTIN_TOOLS,
    // Pre-aprova todas as tools dos servidores meta-ads e omni (servidor + wildcard).
    allowedTools: [
    ...
    canUseTool: async (toolName) => {
      ...
      return {
        behavior: 'deny',
        message: 'Apenas tools dos MCP meta-ads e omni sao permitidas neste runner.',
      };
    },
...
export function collectAssistantMessage(message, state) {
  ...
  for (const block of blocks) {
    ...
    if (block.type === 'tool_use' && typeof block.name === 'string' && startsWithAllowedPrefix(block.name)) {
      // Tool real (Meta ou bridge Omni): registra a acao. A trava anti-invencao
      // (guardReply) conta actions; tool omni executada = dado real do Instagram.
      const prefix = MCP_TOOL_PREFIXES.find((p) => block.name.startsWith(p)) || '';
      const toolName = block.name.slice(prefix.length);
      state.actions.push({ tool: toolName, summary: summarizeToolCall(toolName, block.input), status: 'ok' });
...
export function replyAssertsData(text) {
  return typeof text === 'string' && STRONG_DATA_PATTERNS.some((re) => re.test(text));
}
...
export function guardReply(reply, actions) {
  const usedRealTool = Array.isArray(actions) && actions.length > 0;
...
export function metaToolCountFromInit(message) {
  ...
  return tools.filter((name) => typeof name === 'string' && name.startsWith(MCP_TOOL_PREFIX)).length;
}

// O turno do assistente agora roda na SESSAO UNICA persistente (session.mjs),
// que mantem UMA conexao MCP viva entre login e todos os /run.
