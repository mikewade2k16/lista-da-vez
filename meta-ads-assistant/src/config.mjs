...
export const config = {
  ...
  timeoutMs: parsePositiveInt(process.env.META_ADS_ASSISTANT_TIMEOUT_MS, 120000),
  ...
  mcpUrl: (process.env.META_ADS_MCP_URL || 'https://mcp.facebook.com/ads').trim(),
  ...
  oauthCallbackPort: parsePositiveInt(process.env.META_ADS_OAUTH_CALLBACK_PORT, 8766),
  // Base da API Go que expoe o bridge interno do Instagram (Problema 2).
  bridgeApiBase: (process.env.META_ADS_API_BASE || 'http://localhost:9091').trim(),
...
export function claudeAuthStatus() {
  if ((process.env.CLAUDE_CODE_OAUTH_TOKEN || '').trim() !== '') {
    return { ok: true, detail: 'CLAUDE_CODE_OAUTH_TOKEN presente no ambiente.' };
  }
  ...
  if (existsSync(credentialsPath)) {
    return { ok: true, detail: 'Credenciais do Claude Code encontradas em ~/.claude.' };
  }
  return {
    ok: false,
    detail:
...
//   'oauth'   -> token proprio do runner valido em disco (.auth/tokens.json);
...
export function metaAuthStatus(sessionHasMetaTools) {
