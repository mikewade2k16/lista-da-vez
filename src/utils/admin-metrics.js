...
export function buildConsultantStats({ history, consultantId, monthlyGoal = 0, commissionRate = 0 }) {
  const now = Date.now();
  ...
  const daysWithSales = new Set(
    convertedEntries.filter((entry) => Number(entry.saleAmount || 0) > 0).map((entry) => getDayStamp(entry.finishedAt))
  ).size;
  ...
  return roster
    .map((consultant) => {
      ...
      return {
        ...
        queueJumpServices: entries.filter((entry) => entry.startMode === "queue-jump").length
      };
    })
    .sort((a, b) => {
...
export function buildInsights({ history, visitReasonOptions = [], customerSourceOptions = [] }) {
  ...
  const soldProducts = groupLabels(convertedEntries.map((entry) => entry.productClosed || entry.productSeen || entry.productDetails));
