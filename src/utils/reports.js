...
function evaluateEntryCompletion(entry) {
  const checks = {
    ...
    product: hasText(entry.productClosed) || hasText(entry.productSeen) || hasText(entry.productDetails),
    visitReasons: Array.isArray(entry.visitReasons) && entry.visitReasons.length > 0,
    customerSources: Array.isArray(entry.customerSources) && entry.customerSources.length > 0
  };
...
export function buildReportData({
  ...
  filteredHistory.forEach((entry) => {
    ...
    const product = String(entry.productClosed || "").trim();
    if (product) {
