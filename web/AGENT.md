...
- **Home da agencia:** quando `useCoreAccountStore().activeAccount.isAgency === true` e o papel efetivo inclui o workspace `calendar`, `auth.homeWorkspaceId` deve preferir `calendar` e `auth.homePath` deve ser `/calendario`. Contas-cliente e papeis sem acesso ao calendario preservam a primeira workspace permitida; usuario sem workspace continua em `/perfil`. O login precisa aguardar apenas a hidratacao de accounts necessaria para decidir `isAgency`, mantendo settings/operacao em background.
