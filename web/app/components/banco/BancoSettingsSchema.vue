...
const SETTINGS_TABLES: TableDef[] = [
  {
    ...
    status: 'legacy',
    description:
      'Tabela legada com 91+ colunas cobrindo operacao, modal, alertas e configuracoes diversas. Nao e mais escrita nem lida como fonte autoritativa — todas as secoes foram migradas para as tabelas novas na Fase 9. Mantida apenas como ancora de FK para opcoes e catalogo.',
    ...
    columns: [
      ...
      {
        name: 'max_concurrent_services',
        type: 'int',
        note: 'Limite de atendimentos simultaneos por loja',
      },
      {
        name: 'max_concurrent_services_per_consultant',
        type: 'int',
        nullable: true,
        default: '1',
        note: 'Limite por consultor (COALESCE 1)',
      },
      ...
      {
        name: 'service_cancel_window_seconds',
        type: 'int',
        nullable: true,
        default: '30',
        note: 'Janela de cancelamento (COALESCE 30)',
      },
      ...
      {
        name: 'alert_max_queue_jump_rate',
        type: 'numeric',
        note: 'Alerta: taxa maxima de fora da vez',
      },
      ...
      {
        name: 'finish_flow_mode',
        type: 'text',
        nullable: true,
        default: 'legacy',
        note: 'Modo do modal: legacy | erp-reconciliation',
      },
      ...
      { name: 'product_seen_label', type: 'text', note: 'Label produto visto' },
      {
        name: '...',
        type: '...',
        note: '74 campos adicionais de labels, placeholders, show/hide, required e modos de selecao do modal',
      },
  ...
  },
  {
    ...
    status: 'nova',
    description:
      'Tabela separada para configuracoes operacionais estaveis e tipadas. Criada na Fase 3 com backfill. Ativada com dual-read/write na Fase 6. Leitura e escrita exclusivas desde a Fase 9 — sem fallback legacy. Scan por nome com pgx.RowToStructByName (Fase 8).',
    phase: 'Fase 3 criada | Fase 6 dual-write | Fase 9 exclusiva',
    columns: [
      { name: 'tenant_id', type: 'uuid', note: 'PK — chave do tenant' },
      {
        name: 'selected_operation_template_id',
        type: 'text',
        note: 'Template operacional selecionado',
      },
      { name: 'max_concurrent_services', type: 'int', note: 'Limite de atendimentos simultaneos' },
      {
        ...
        type: 'int',
        default: '1',
        note: 'Limite por consultor',
      },
      {
        name: 'timing_fast_close_minutes',
        type: 'int',
        note: 'Tempo para atendimento rapido (minutos)',
      },
      {
        name: 'timing_long_service_minutes',
        type: 'int',
        note: 'Tempo para atendimento longo (minutos)',
      },
      { name: 'timing_low_sale_amount', type: 'numeric', note: 'Valor minimo de ticket baixo' },
      {
        ...
        type: 'int',
        default: '30',
        note: 'Janela de cancelamento (segundos)',
      },
      { name: 'test_mode_enabled', type: 'bool', default: 'false', note: 'Modo de teste ativo' },
      {
        name: 'auto_fill_finish_modal',
        type: 'bool',
        default: 'false',
        note: 'Auto-preenchimento do modal de encerramento',
      },
      {
        name: 'updated_by',
        type: 'uuid',
        nullable: true,
        note: 'ID do usuario que fez a ultima alteracao',
      },
      { name: 'updated_at', type: 'timestamptz', note: 'Ultima atualizacao' },
    ],
    relations: [
      {
        from: 'tenant_id',
        to: 'tenant_operation_settings.tenant_id',
        label: 'ancora de FK para opcoes/catalogo (sem leitura/escrita desde Fase 9)',
      },
    ],
  },
  {
    ...
    status: 'nova',
    description:
      'Configuracao do modal de encerramento em documento jsonb versionado. Criada na Fase 3, ativada com dual-read/write na Fase 4. Leitura e escrita exclusivas desde a Fase 9. A coluna finish_flow_mode e autoritativa sobre o jsonb. Aplicacao de template via endpoint transacional (Fase 7).',
    ...
    columns: [
      { name: 'tenant_id', type: 'uuid', note: 'PK — chave do tenant' },
      {
        ...
        type: 'text',
        default: 'legacy',
        note: 'Modo do fluxo de encerramento: legacy | erp-reconciliation. Prevalece sobre o jsonb.',
      },
      {
        name: 'schema_version',
        type: 'int',
        default: '1',
        note: 'Versao do schema do config jsonb. v1 cobre todos os 76 campos do modal.',
      },
      {
        name: 'config',
        type: 'jsonb',
        note: 'Documento camelCase com labels, placeholders, show/hide, required e modos de selecao. Chaves alinham com json.Marshal do ModalConfig Go.',
      },
      {
        name: 'updated_by',
        type: 'uuid',
    ...
    relations: [
      {
        from: 'tenant_id',
        to: 'tenant_operation_settings.tenant_id',
        label: 'ancora de FK (sem leitura/escrita desde Fase 9)',
      },
      {
        from: 'config (jsonb)',
        to: 'ModalConfig (Go struct)',
        label: 'json.Marshal/Unmarshal — camelCase',
      },
    ],
  },
  {
    ...
    status: 'nova',
    description:
      'Thresholds de alertas operacionais separados do core. Criada na Fase 3, ativada com dual-read/write na Fase 5. Leitura e escrita exclusivas desde a Fase 9. Os 4 campos de alerta sao sobrepostos pelo GetOperationSection. Scan por nome com pgx.RowToStructByName (Fase 8).',
    phase: 'Fase 3 criada | Fase 5 dual-write | Fase 9 exclusiva',
    columns: [
      { name: 'tenant_id', type: 'uuid', note: 'PK — chave do tenant' },
      {
        name: 'alert_min_conversion_rate',
        type: 'numeric',
        note: 'Taxa minima de conversao esperada (0-1)',
      },
      {
        name: 'alert_max_queue_jump_rate',
        type: 'numeric',
        note: 'Taxa maxima de fora da vez tolerada (0-1)',
      },
      {
        name: 'alert_min_pa_score',
        type: 'numeric',
        note: 'PA (pontuacao de atendimento) minima esperada',
      },
      {
        name: 'alert_min_ticket_average',
        type: 'numeric',
        note: 'Ticket medio minimo esperado (R$)',
      },
      {
        name: 'updated_by',
        type: 'uuid',
  ...
  },
  {
    ...
    status: 'relacional',
    description:
      'Catalogo normalizado de opcoes configuráveis por tenant. Cada grupo (kind) forma uma lista ordenada de itens. Permanece como tabela relacional independente.',
    phase: 'estavel',
    columns: [
      ...
      { name: 'tenant_id', type: 'uuid', note: 'Escopo do tenant' },
      {
        name: 'kind',
        type: 'text',
        note: 'Grupo: visit_reason | customer_source | pause_reason | queue_jump_reason | loss_reason | profession',
      },
    ...
    relations: [{ from: 'tenant_id', to: 'tenants.id', label: 'escopo por tenant' }],
  },
  {
    ...
    status: 'catalogo',
    description:
      'Catalogo administrativo de produtos configurado manualmente por tenant. Diferente do catalogo ERP (erp_item_current), este e de curadoria manual para o modal de atendimento.',
    phase: 'estavel',
    columns: [
  ...
  },
  {
    ...
    status: 'legacy',
    description:
      'Tabela legada anterior a migracao para escopo por tenant. Mantida como fonte de backfill para tenants que ainda nao tem linha em tenant_operation_settings. Nao e mais escrita.',
    phase: 'pre-legado',
    columns: [
      { name: 'store_id', type: 'bigint', note: 'PK — ID da loja (modelo antigo)' },
      {
        name: '...',
        type: '...',
        note: 'Mesmos campos de tenant_operation_settings, mas scoped por store_id',
      },
      { name: 'updated_at', type: 'timestamptz', note: 'Ultima atualizacao' },
    ],
  },
  {
    ...
    status: 'legacy',
    description:
      'Tabela legada de opcoes scoped por loja. Mantida como fonte de backfill. Nao e mais escrita apos a migracao para tenant_setting_options.',
    phase: 'pre-legado',
    columns: [
  ...
  },
  {
    ...
    status: 'legacy',
    description:
      'Tabela legada de catalogo manual scoped por loja. Mantida como fonte de backfill. Nao e mais escrita apos a migracao para tenant_catalog_products.',
    phase: 'pre-legado',
    columns: [
      { name: 'id', type: 'text', note: 'PK' },
      { name: 'store_id', type: 'bigint', note: 'Escopo da loja (modelo antigo)' },
...
const expandedTables = ref<Set<string>>(
  new Set([
...
<template>
  <section class="banco-schema">
    <div class="banco-schema__legend">
      ...
      <span class="banco-schema__chip-label">
        Tabela criada na refatoracao — em fase de ativacao
      </span>
      <span class="banco-schema__chip banco-schema__chip--estavel">estavel</span>
      <span class="banco-schema__chip-label">
        Tabela madura, sem alteracoes estruturais previstas
      </span>
    ...
    <div class="banco-schema__migration-status">
      <div class="banco-schema__migration-item">
        <span
          class="material-icons-round banco-schema__migration-icon banco-schema__migration-icon--done"
        >
          check_circle
        </span>
        <div>
      ...
      <div class="banco-schema__migration-item banco-schema__migration-item--highlight">
        ...
        <div>
          ...
          <span>
            Corte — tenant_operation_settings nao e mais lida/escrita para modal, core ou alertas
          </span>
        </div>
      </div>
    </div>
    ...
    <div class="banco-schema__tables">
      ...
      >
        <button type="button" class="banco-schema__table-head" @click="toggleTable(table.name)">
          <div class="banco-schema__table-head-left">
            <span
              :class="[
                'material-icons-round',
                'banco-schema__chevron',
                expandedTables.has(table.name) ? 'banco-schema__chevron--open' : '',
              ]"
            >
              chevron_right
            </span>
            <code class="banco-schema__table-name">{{ table.name }}</code>
            <span :class="`banco-schema__chip banco-schema__chip--${table.status}`">
        ...
        <div v-if="expandedTables.has(table.name)" class="banco-schema__table-body">
          ...
          <div class="banco-schema__columns">
            ...
            <div v-for="col in table.columns" :key="col.name" class="banco-schema__column-row">
              <code class="banco-schema__col-name">{{ col.name }}</code>
              <span class="banco-schema__col-type">
                <code>{{ col.type }}</code>
                <span v-if="col.nullable" class="banco-schema__col-tag banco-schema__col-tag--null">
                  nullable
                </span>
                <span
                  v-if="col.default"
                  class="banco-schema__col-tag banco-schema__col-tag--default"
                >
                  default {{ col.default }}
                </span>
              </span>
