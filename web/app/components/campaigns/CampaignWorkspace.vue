...
const canEditCampaigns = computed(() =>
  canManageCampaigns(auth.role, auth.permissionKeys, auth.permissionsResolved),
)
...
const scopedHistoryEntries = computed(() => {
  ...
  return historyEntries.value.filter(
    (entry) => String(entry?.storeId || '').trim() === storeScopeFilter.value,
  )
})
const campaignStats = computed(() => {
  const statsByCampaignId = new Map(
    (props.state.campaigns || []).map((campaign) => [campaign.id, { hits: 0, bonus: 0 }]),
  )
  ...
  (props.state.productCatalog || [])
    ...
    .map((product) => ({
      value: String(product.code || '')
        .trim()
        .toUpperCase(),
      label: `${product.name} (${String(product.code || '')
        .trim()
        .toUpperCase()})`,
    })),
)
...
watch(
  ...
  (campaigns) => {
    drafts.value = Object.fromEntries(
      (campaigns || []).map((campaign) => [campaign.id, buildDraft(campaign)]),
    )
  },
...
watch(
  ...
  () => {
    ...
    const exists = storeFilterOptions.value.some(
      (option) => option.value === storeScopeFilter.value,
    )
    if (!exists) {
      storeScopeFilter.value = 'all'
    }
...
async function saveCampaign(campaignId) {
  const result = await campaignsStore.updateCampaign(
    campaignId,
    normalizeCampaign(drafts.value[campaignId]),
  )
  ...
  <section class="admin-panel" data-testid="campaigns-panel">
    <header class="admin-panel__header">
      ...
      <p class="admin-panel__text">
        Regras aplicadas automaticamente no fechamento para auditoria e bonus.
      </p>
      <p v-if="integratedScope" class="admin-panel__text">
        Escopo consolidado das lojas acessiveis, com filtro local por loja para comparar a tracao
        das campanhas.
      </p>
    </header>
    ...
    <section class="metric-grid metric-grid--tight" data-testid="campaigns-summary">
      <article class="metric-card">
        ...
        <strong class="metric-card__value">{{ state.campaigns.length }}</strong>
      </article>
      <article class="metric-card">
        <span class="metric-card__label">Campanhas ativas</span>
        <strong class="metric-card__value">{{ activeCampaignCount }}</strong>
      </article>
      <article class="metric-card">
    ...
    <div class="campaign-type-filter">
      <button
        data-testid="campaigns-filter-todas"
        :class="['campaign-type-filter__btn', typeFilter === 'todas' && 'is-active']"
        type="button"
        @click="typeFilter = 'todas'"
      >
        Todas
      </button>
      <button
        data-testid="campaigns-filter-interna"
        :class="['campaign-type-filter__btn', typeFilter === 'interna' && 'is-active']"
        type="button"
        @click="typeFilter = 'interna'"
      >
        Internas
      </button>
      <button
        data-testid="campaigns-filter-comercial"
        :class="['campaign-type-filter__btn', typeFilter === 'comercial' && 'is-active']"
        type="button"
        @click="typeFilter = 'comercial'"
      >
        Comerciais
      </button>
    </div>
    ...
    <form
      v-if="canEditCampaigns"
      class="settings-card campaign-card"
      data-testid="campaigns-new-form"
      @submit.prevent="createCampaign"
    >
      <header class="settings-card__header">
        <h3 class="settings-card__title">Nova campanha</h3>
        <p class="settings-card__text">
          Cadastro completo da regra comercial. Produtos vinculados entram automaticamente quando o
          codigo vendido bater.
        </p>
      </header>
      ...
      <div class="campaign-grid">
        <label class="settings-field">
          <span>Nome</span>
          <input v-model="newCampaign.name" type="text" data-testid="campaigns-new-name" />
        </label>
        <label class="settings-field">
          <span>Descricao</span>
          <input v-model="newCampaign.description" type="text" />
        </label>
        <label class="settings-field">
          <span>Tipo</span>
          <AppSelectField
        ...
        <label class="settings-field">
          <span>Inicio</span>
          <input v-model="newCampaign.startsAt" type="date" />
        </label>
        <label class="settings-field">
          <span>Fim</span>
          <input v-model="newCampaign.endsAt" type="date" />
        </label>
        <label class="settings-field">
        ...
        <label class="settings-field">
          ...
          <input
            :value="newCampaign.minSaleAmount"
            type="number"
            min="0"
            step="1"
            @input="updateNewCampaignField('minSaleAmount', $event.target.value)"
          />
        </label>
        <label class="settings-field">
          <span>Duracao maxima (min)</span>
          <input
            :value="newCampaign.maxServiceMinutes"
            type="number"
            min="0"
            step="1"
            @input="updateNewCampaignField('maxServiceMinutes', $event.target.value)"
          />
        </label>
        <label class="settings-field">
          <span>Bonus fixo (R$)</span>
          <input
            :value="newCampaign.bonusFixed"
            type="number"
            min="0"
            step="0.01"
            @input="updateNewCampaignField('bonusFixed', $event.target.value)"
          />
        </label>
        <label class="settings-field">
          ...
          <input
            ...
            max="1"
            step="0.001"
            @input="updateNewCampaignField('bonusRate', $event.target.value)"
          />
        </label>
      </div>
      ...
      <div class="campaign-grid campaign-grid--toggles">
        <label class="settings-toggle">
          <input
            ...
            type="checkbox"
            @change="updateNewCampaignField('isActive', $event.target.checked)"
          />
          <span>Campanha ativa</span>
        </label>
        <label class="settings-toggle">
          <input
            :checked="newCampaign.queueJumpOnly"
            type="checkbox"
            @change="updateNewCampaignField('queueJumpOnly', $event.target.checked)"
          />
      ...
      <div class="campaign-grid campaign-grid--options">
        <div class="settings-field">
          ...
          <div class="campaign-option-list">
            <label
              v-for="option in state.customerSourceOptions"
              :key="option.id"
              class="settings-toggle"
            >
              <input
                type="checkbox"
                :checked="newCampaign.sourceIds.includes(option.id)"
                @change="toggleNewCampaignListValue('sourceIds', option.id)"
              />
        ...
        <div class="settings-field">
          ...
          <div class="campaign-option-list">
            <label
              v-for="option in state.visitReasonOptions"
              :key="option.id"
              class="settings-toggle"
            >
              <input
                type="checkbox"
                :checked="newCampaign.reasonIds.includes(option.id)"
                @change="toggleNewCampaignListValue('reasonIds', option.id)"
              />
              <span>{{ option.label }}</span>
            </label>
          </div>
        ...
        <div class="settings-field">
          ...
          <span v-if="!campaignProductOptions.length" class="settings-card__text">
            Cadastre codigos em Configuracoes &gt; Produtos para automatizar por item.
          </span>
          <div v-else class="campaign-option-list">
            <label
              v-for="option in campaignProductOptions"
              :key="option.value"
              class="settings-toggle"
            >
              <input
                type="checkbox"
                :checked="newCampaign.productCodes.includes(option.value)"
                @change="toggleNewCampaignListValue('productCodes', option.value)"
              />
              <span>{{ option.label }}</span>
            </label>
          </div>
      ...
      <div class="report-actions">
        <button class="option-add__button" type="submit" data-testid="campaigns-new-submit">
          Criar campanha
        </button>
      </div>
    </form>
    ...
    <div class="settings-grid campaign-list" data-testid="campaigns-list">
      <article v-if="!filteredCampaigns.length" class="settings-card">
        <span class="insight-empty">
          {{
            state.campaigns.length
              ? 'Nenhuma campanha nessa categoria.'
      ...
      >
        <header class="settings-card__header">
          <div class="campaign-card__title-row">
            ...
            <span :class="['campaign-status', STATUS_CLASS[statusOf(campaign)]]">
              {{ STATUS_LABEL[statusOf(campaign)] }}
            </span>
            <span class="insight-tag insight-tag--sm">
              {{ campaign.campaignType === 'comercial' ? 'Comercial' : 'Interna' }}
            </span>
          </div>
        ...
        <div class="insight-time-grid">
          <span class="insight-tag">
            Aplicacoes:
            <strong>{{ campaignStats.get(campaign.id)?.hits || 0 }}</strong>
          </span>
          <span class="insight-tag">
            Bonus total:
            <strong>{{ formatCurrency(campaignStats.get(campaign.id)?.bonus || 0) }}</strong>
          </span>
        </div>
        ...
        <template v-if="performance.get(campaign.id)?.hasPeriod">
          <div class="campaign-perf">
            <div class="campaign-perf__col campaign-perf__col--hit">
              ...
              <strong class="campaign-perf__value">
                {{ performance.get(campaign.id).hit.total }} atend.
              </strong>
              <span class="campaign-perf__sub">
                {{ performance.get(campaign.id).hit.conversionRate.toFixed(1) }}% conv. ·
                {{ formatCurrency(performance.get(campaign.id).hit.ticketAverage) }} ticket
              </span>
            </div>
            ...
            <div class="campaign-perf__col campaign-perf__col--nohit">
              ...
              <strong class="campaign-perf__value">
                {{ performance.get(campaign.id).nonHit.total }} atend.
              </strong>
              <span class="campaign-perf__sub">
                {{ performance.get(campaign.id).nonHit.conversionRate.toFixed(1) }}% conv. ·
                {{ formatCurrency(performance.get(campaign.id).nonHit.ticketAverage) }} ticket
              </span>
            </div>
          </div>
        </template>
        ...
        <div class="campaign-grid">
          <label class="settings-field">
            <span>Nome</span>
            <input
              :value="drafts[campaign.id]?.name || ''"
              type="text"
              :disabled="!canEditCampaigns"
              @input="updateDraftField(campaign.id, 'name', $event.target.value)"
            />
          </label>
          <label class="settings-field">
            <span>Descricao</span>
            <input
              :value="drafts[campaign.id]?.description || ''"
              type="text"
              :disabled="!canEditCampaigns"
              @input="updateDraftField(campaign.id, 'description', $event.target.value)"
            />
          </label>
          <label class="settings-field">
            <span>Tipo</span>
          ...
          <label class="settings-field">
            <span>Inicio</span>
            <input
              :value="drafts[campaign.id]?.startsAt || ''"
              type="date"
              :disabled="!canEditCampaigns"
              @input="updateDraftField(campaign.id, 'startsAt', $event.target.value)"
            />
          </label>
          <label class="settings-field">
            <span>Fim</span>
            <input
              :value="drafts[campaign.id]?.endsAt || ''"
              type="date"
              :disabled="!canEditCampaigns"
              @input="updateDraftField(campaign.id, 'endsAt', $event.target.value)"
            />
          </label>
          <label class="settings-field">
            <span>Desfecho alvo</span>
          ...
          <label class="settings-field">
            <span>Venda minima (R$)</span>
            <input
              :value="drafts[campaign.id]?.minSaleAmount || 0"
              type="number"
              min="0"
              step="1"
              :disabled="!canEditCampaigns"
              @input="updateDraftField(campaign.id, 'minSaleAmount', $event.target.value)"
            />
          </label>
          <label class="settings-field">
            <span>Duracao maxima (min)</span>
            <input
              :value="drafts[campaign.id]?.maxServiceMinutes || 0"
              type="number"
              min="0"
              step="1"
              :disabled="!canEditCampaigns"
              @input="updateDraftField(campaign.id, 'maxServiceMinutes', $event.target.value)"
            />
          </label>
          <label class="settings-field">
            <span>Bonus fixo (R$)</span>
            <input
              :value="drafts[campaign.id]?.bonusFixed || 0"
              type="number"
              min="0"
              step="0.01"
              :disabled="!canEditCampaigns"
              @input="updateDraftField(campaign.id, 'bonusFixed', $event.target.value)"
            />
          </label>
          <label class="settings-field">
            <span>Bonus percentual</span>
            <input
              :value="drafts[campaign.id]?.bonusRate || 0"
              type="number"
              min="0"
              max="1"
              step="0.001"
              :disabled="!canEditCampaigns"
              @input="updateDraftField(campaign.id, 'bonusRate', $event.target.value)"
            />
          </label>
        </div>
        ...
        <div class="campaign-grid campaign-grid--toggles">
          <label class="settings-toggle">
            <input
              type="checkbox"
              :checked="Boolean(drafts[campaign.id]?.isActive)"
              :disabled="!canEditCampaigns"
              @change="updateDraftField(campaign.id, 'isActive', $event.target.checked)"
            />
            <span>Campanha ativa</span>
          </label>
          <label class="settings-toggle">
            <input
              type="checkbox"
              :checked="Boolean(drafts[campaign.id]?.queueJumpOnly)"
              :disabled="!canEditCampaigns"
              @change="updateDraftField(campaign.id, 'queueJumpOnly', $event.target.checked)"
            />
            <span>Somente fora da vez</span>
          </label>
        </div>
          ...
          <div class="settings-field">
            ...
            <div class="campaign-option-list">
              ...
              >
                <input
                  type="checkbox"
                  :checked="Boolean(drafts[campaign.id]?.sourceIds?.includes(option.id))"
                  :disabled="!canEditCampaigns"
                  @change="toggleDraftListValue(campaign.id, 'sourceIds', option.id)"
                />
                <span>{{ option.label }}</span>
              </label>
            </div>
          ...
          <div class="settings-field">
            ...
            <div class="campaign-option-list">
              ...
              >
                <input
                  type="checkbox"
                  :checked="Boolean(drafts[campaign.id]?.reasonIds?.includes(option.id))"
                  :disabled="!canEditCampaigns"
                  @change="toggleDraftListValue(campaign.id, 'reasonIds', option.id)"
                />
                <span>{{ option.label }}</span>
              </label>
            </div>
          ...
          <div class="settings-field">
            ...
            <div v-else class="campaign-option-list">
              ...
              >
                <input
                  type="checkbox"
                  :checked="Boolean(drafts[campaign.id]?.productCodes?.includes(option.value))"
                  :disabled="!canEditCampaigns"
                  @change="toggleDraftListValue(campaign.id, 'productCodes', option.value)"
                />
                <span>{{ option.label }}</span>
              </label>
            </div>
        ...
        <div v-if="canEditCampaigns" class="report-actions">
          ...
          <button class="option-row__remove" type="button" @click="removeCampaign(campaign.id)">
            Excluir campanha
          </button>
        </div>
      </form>
