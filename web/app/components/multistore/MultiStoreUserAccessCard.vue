...
const genericRoleCatalog = computed(() =>
  roleCatalog.value.filter((role) => role.id !== 'consultant'),
)
...
function isConsultantManaged(user) {
  return (
    String(user?.managedBy || '').trim() === 'consultants' ||
    String(user?.role || '').trim() === 'consultant'
  )
}
...
function getScopedStoreOptions(tenantId) {
  ...
  return allStoreOptions.value.filter(
    (store) => !normalizedTenantId || store.tenantId === normalizedTenantId,
  )
}

function getOnboardingLabel(user) {
  if (user?.onboarding?.mustChangePassword) return 'Troca de senha pendente'
...
function getOnboardingTone(user) {
  if (user?.onboarding?.mustChangePassword) return 'insight-tag insight-tag--warning'
  const status = String(user?.onboarding?.status || '').trim()
...
async function submitCreate() {
  ...
  if (createdMode === 'password' && !result?.invitation?.inviteUrl) {
    ui.success(
      'Usuario criado com senha temporaria. No primeiro acesso ele precisara trocar a senha.',
    )
    return
  }
  ...
  <article v-if="canEditUsers" class="settings-card" data-testid="multistore-users-card">
    <header class="settings-card__header">
      ...
      <p class="settings-card__text">
        Controle de contas, papeis, escopo e onboarding por convite. Definicao e reset manual de
        senha ficam restritos ao dev por enquanto.
      </p>
    </header>
          ...
          <template v-for="user in usersStore.users" :key="user.id">
            <tr>
              ...
              <td>
                {{
                  ...
                  user.role === 'store_terminal'
                    ...
                    : user.tenantId || 'Plataforma'
                }}
              </td>
              <td>
                <span :class="getOnboardingTone(user)">{{ getOnboardingLabel(user) }}</span>
                <p v-if="isConsultantManaged(user)" class="settings-card__text">
                  Gerenciado pela aba Consultores.
                </p>
              </td>
              <td>
              ...
              <td>
                <button
                  v-if="
                    user.active && user.onboarding?.status !== 'ready' && !isConsultantManaged(user)
                  "
                ...
                <button
                  v-if="canManagePasswords && user.active && user.onboarding?.hasPassword"
                  class="option-row__save"
                  type="button"
              ...
              <td colspan="7">
                <form class="multistore-form multistore-form--add" @submit.prevent="submitUpdate">
                  ...
                  <div v-if="isStoreScoped(editDraft.role)" class="multistore-user__scope-grid">
                    <label
                      v-for="store in getScopedStoreOptions(editDraft.tenantId)"
                      :key="store.id"
                  ...
                  <p v-if="isSingleStoreScoped(editDraft.role)" class="settings-card__text">
                    Esse papel fica vinculado a uma unica loja.
                  </p>
                  <div class="multistore-form__actions">
                    ...
                    <button class="product-row__remove" type="button" @click="editingUserId = ''">
                      Cancelar
                    </button>
                  </div>
                </form>
    ...
    <form class="multistore-form multistore-form--add" @submit.prevent="submitCreate">
      ...
      <div v-if="isStoreScoped(createDraft.role)" class="multistore-user__scope-grid">
        <label
          v-for="store in getScopedStoreOptions(createDraft.tenantId)"
          :key="store.id"
          class="settings-toggle"
        >
          <input
      ...
      <p v-if="isSingleStoreScoped(createDraft.role)" class="settings-card__text">
        Esse papel fica vinculado a uma unica loja.
      </p>
      <div class="multistore-form__actions">
        <label class="settings-toggle">
      ...
      <p class="settings-card__text">
        Consultores devem ser criados na gestao de consultores para nascerem com roster e login
        vinculados.
      </p>
      <p v-if="!canManagePasswords" class="settings-card__text">
        Para perfis nao-dev, o onboarding segue somente por convite.
      </p>
    </form>
  </article>
</template>
