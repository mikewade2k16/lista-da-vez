...
const archivedManagedStores = computed(() =>
  managedStores.value.filter((store) => store.isActive === false),
)
  ...
  <section class="admin-panel" data-testid="multistore-panel">
    <header class="admin-panel__header">
      ...
      <p class="admin-panel__text">
        Comparativo operacional para acompanhar performance entre lojas.
      </p>
    </header>
    ...
    <section class="metric-grid" data-testid="multistore-summary">
      <article class="metric-card">
        ...
        <strong class="metric-card__value">{{ summary.activeStores }}</strong>
      </article>
      <article class="metric-card">
        <span class="metric-card__label">Atendimentos consolidados</span>
        <strong class="metric-card__value">{{ summary.totalAttendances }}</strong>
      </article>
      <article class="metric-card">
      ...
      <article class="metric-card">
        ...
        <strong class="metric-card__value">{{ summary.totalQueue }}</strong>
      </article>
      <article class="metric-card">
        <span class="metric-card__label">Em atendimento agora</span>
        <strong class="metric-card__value">{{ summary.totalActiveServices }}</strong>
      </article>
      <article class="metric-card">
        <span class="metric-card__label">Score medio operacional</span>
        <strong class="metric-card__value">{{ Math.round(summary.averageHealthScore) }}</strong>
      </article>
    </section>
        ...
        <table class="insight-table">
          ...
          <tbody>
            ...
            <tr v-for="row in rows" :key="row.storeId">
              ...
              <td>
                <span
                  v-if="row.monthlyGoal"
                  :class="
                    row.soldValue >= row.monthlyGoal
                      ? 'multistore-goal--hit'
                      : 'multistore-goal--miss'
                  "
                >
              ...
              <td>
                ...
                <button
                  ...
                  type="button"
                  @click="multiStore.setActiveStore(row.storeId)"
                >
                  Abrir
                </button>
              </td>
            ...
            <div class="meeting-goal-row">
              ...
              <div class="meeting-goal-row__bar">
                <div class="meeting-goal-row__track">
                  <div
                    :class="[
                      'meeting-goal-row__fill',
                      row.monthlyGoal && row.soldValue >= row.monthlyGoal
                        ? 'meeting-goal-row__fill--hit'
                        : 'meeting-goal-row__fill--miss',
                    ]"
                    :style="{
                      width: row.monthlyGoal
                        ? `${Math.min(100, (row.soldValue / row.monthlyGoal) * 100).toFixed(1)}%`
                        : '0%',
                    }"
                  ></div>
                </div>
                <span
                  :class="[
                    'meeting-goal-row__value',
                    row.monthlyGoal && row.soldValue >= row.monthlyGoal
                      ? 'meeting-goal-row__value--hit'
                      : 'meeting-goal-row__value--miss',
                  ]"
                >
                  {{
                    row.monthlyGoal
                      ? formatPercent((row.soldValue / row.monthlyGoal) * 100)
              ...
              <div class="meeting-goal-row__bar">
                <div class="meeting-goal-row__track">
                  <div
                    :class="[
                      'meeting-goal-row__fill',
                      row.conversionGoal && row.conversionRate >= row.conversionGoal
                        ? 'meeting-goal-row__fill--hit'
                        : 'meeting-goal-row__fill--miss',
                    ]"
                    :style="{
                      width: row.conversionGoal
                        ? `${Math.min(100, (row.conversionRate / row.conversionGoal) * 100).toFixed(1)}%`
                        : `${Math.min(100, row.conversionRate).toFixed(1)}%`,
                    }"
                  ></div>
                </div>
                <span
              ...
              <div class="meeting-goal-row__bar">
                <div class="meeting-goal-row__track">
                  <div
                    :class="[
                      'meeting-goal-row__fill',
                      row.avgTicketGoal && row.ticketAverage >= row.avgTicketGoal
                        ? 'meeting-goal-row__fill--hit'
                        : 'meeting-goal-row__fill--miss',
                    ]"
                    :style="{
                      width: row.avgTicketGoal
                        ? `${Math.min(100, (row.ticketAverage / row.avgTicketGoal) * 100).toFixed(1)}%`
                        : '0%',
                    }"
                  ></div>
                </div>
              ...
              <div class="meeting-goal-row__bar">
                <div class="meeting-goal-row__track">
                  <div
                    :class="[
                      'meeting-goal-row__fill',
                      row.paGoal && row.paScore >= row.paGoal
                        ? 'meeting-goal-row__fill--hit'
                        : 'meeting-goal-row__fill--miss',
                    ]"
                    :style="{
                      width: row.paGoal
                        ? `${Math.min(100, (row.paScore / row.paGoal) * 100).toFixed(1)}%`
                        : '0%',
                    }"
                  ></div>
                </div>
              ...
              <div class="meeting-goal-row__bar">
                <div class="meeting-goal-row__track">
                  <div
                    ...
                    :style="{
                      width: summary.totalAttendances
                        ? `${((row.attendances / summary.totalAttendances) * 100).toFixed(1)}%`
                        : '0%',
                    }"
                  ></div>
                </div>
                <span class="meeting-goal-row__value meeting-goal-row__value--hit">
                  {{ row.attendances }}
                </span>
              </div>
            </div>
    ...
    <article v-if="canEditStores" class="settings-card">
      ...
      <div class="option-list">
        ...
        >
          <div class="multistore-form__row">
            <input
              v-model="storeDrafts[store.id].name"
              class="product-row__input"
              type="text"
              placeholder="Nome"
            />
            <input
              v-model="storeDrafts[store.id].code"
              class="product-row__input"
              type="text"
              placeholder="Codigo"
            />
            <input
              v-model="storeDrafts[store.id].city"
              class="product-row__input"
              type="text"
              placeholder="Cidade"
            />
          ...
          <div class="multistore-form__row">
            <label class="multistore-form__field">
              ...
              <input
                v-model="storeDrafts[store.id].monthlyGoal"
                class="product-row__input"
                type="number"
                min="0"
                step="100"
            ...
            <label class="multistore-form__field">
              ...
              <input
                v-model="storeDrafts[store.id].weeklyGoal"
                class="product-row__input"
                type="number"
                min="0"
                step="100"
            ...
            <label class="multistore-form__field">
              ...
              <input
                v-model="storeDrafts[store.id].avgTicketGoal"
                class="product-row__input"
                type="number"
                min="0"
                step="100"
            ...
            <label class="multistore-form__field">
              ...
              <input
                v-model="storeDrafts[store.id].conversionGoal"
                class="product-row__input"
                type="number"
                min="0"
                max="100"
            ...
            <label class="multistore-form__field">
              ...
              <input
                v-model="storeDrafts[store.id].paGoal"
                class="product-row__input"
                type="number"
                min="0"
                step="0.1"
          ...
          <div class="multistore-form__actions">
            ...
            <button class="product-row__remove" type="button" @click="archiveStore(store.id)">
              Arquivar
            </button>
            <button class="product-row__remove" type="button" @click="deleteStore(store)">
              Excluir
            </button>
          </div>
        </form>
      ...
      <div v-if="archivedManagedStores.length" class="option-list">
        <article v-for="store in archivedManagedStores" :key="store.id" class="option-row">
          ...
          <div class="multistore-form__actions">
            <button class="option-row__save" type="button" @click="restoreStore(store.id)">
              Restaurar
            </button>
            <button class="product-row__remove" type="button" @click="deleteStore(store)">
              Excluir
      ...
      <form class="multistore-form multistore-form--add" @submit.prevent="createStore">
        <div class="multistore-form__row">
          <input
            ...
            data-testid="multistore-new-name"
          />
          <input
        ...
        <div class="multistore-form__row">
          ...
          <label class="multistore-form__field">
            <span class="multistore-form__label">Ticket médio alvo (R$)</span>
            <input
              v-model="newStore.avgTicketGoal"
              class="product-add__input"
              type="number"
              min="0"
              step="100"
          ...
          <label class="multistore-form__field">
            <span class="multistore-form__label">Conversão alvo (%)</span>
            <input
              v-model="newStore.conversionGoal"
              class="product-add__input"
              type="number"
              min="0"
              max="100"
        ...
        <div class="multistore-form__actions">
          ...
          <button class="product-add__button" type="submit" data-testid="multistore-new-submit">
            Adicionar loja
          </button>
        </div>
      </form>
