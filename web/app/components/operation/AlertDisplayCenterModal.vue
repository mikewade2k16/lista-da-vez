...
<template>
  <div v-if="primaryAlert" class="center-modal-overlay">
    <div
      class="center-modal"
      :style="{ '--gradient': getColorGradient(primaryAlert.colorTheme || 'amber') }"
    >
      <div class="center-modal__header">
        <h2 class="center-modal__title">
          {{
            renderTemplate(
              primaryAlert.headline || primaryAlert.titleTemplate || 'Alerta operacional',
              primaryAlert,
            )
          }}
        </h2>
        <button
          v-if="!primaryAlert.isMandatory && primaryAlert.interactionKind === 'dismiss'"
          class="center-modal__close"
          @click="handleDismiss"
        >
          ✕
        </button>
      ...
      <p class="center-modal__body">
        {{ renderTemplate(primaryAlert.body || primaryAlert.bodyTemplate || '', primaryAlert) }}
      </p>
      ...
      <div class="center-modal__footer">
        ...
        <template v-else-if="primaryAlert.interactionKind === 'dismiss'">
          <button
            class="center-modal__action-btn center-modal__action-btn--primary"
            :disabled="respondingToId === primaryAlert.id"
            @click="handleDismiss"
          >
