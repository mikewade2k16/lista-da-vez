...
import OperationAlertBanner from '~/components/operation/OperationAlertBanner.vue'
import AlertDisplayCornerPopup from '~/components/operation/AlertDisplayCornerPopup.vue'
import AlertDisplayCenterModal from '~/components/operation/AlertDisplayCenterModal.vue'
import AlertDisplayFullscreen from '~/components/operation/AlertDisplayFullscreen.vue'
  ...
  const overviewActiveServices = Array.isArray(overview?.activeServices)
    ? overview.activeServices.filter(
        (service: Record<string, any>) =>
          String(service?.storeId || '').trim() === normalizedStoreId,
      )
    : []
  if (overview) {
    return {
