...
const canStartParallel = computed(() => {
  return (
    !props.readOnly &&
    !props.integratedMode &&
    totalOpenServices.value < props.maxConcurrentPerConsultant
  )
})

function startedAtLabel(service) {
  return formatClock(
    Math.max(0, Number(service?.serviceStartedAt || 0) - Number(props.serverClockOffsetMs || 0)),
  )
}
    ...
    <div class="service-card__summary">
      ...
      <div class="service-card__content">
        <span class="queue-card__headline">
          ...
          <span v-if="primaryStatusLabel" class="service-card__status-badge">
            {{ primaryStatusLabel }}
          </span>
          <span v-if="integratedMode && primaryService?.storeName" class="queue-card__store-badge">
            {{ primaryService.storeName }}
          </span>
        </span>
      </div>

      <span
        v-if="activeServiceAlert"
        class="service-card__alert-badge"
        :title="activeServiceAlertTitle"
      >
    ...
    <div class="service-card__services" :style="{ '--service-columns': serviceColumns }">
      ...
      >
        ...
        <div
          ...
          :class="{
            'service-card__row-actions--parallel':
              canStartParallel && index === services.length - 1,
          }"
        >
          <button
