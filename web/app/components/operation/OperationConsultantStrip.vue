...
import OperationPauseReasonDialog from '~/components/operation/OperationPauseReasonDialog.vue'
...
const waitingIds = computed(
  () => new Set((props.state.waitingList || []).map((person) => person.id)),
)
const activeServiceIds = computed(
  () => new Set((props.state.activeServices || []).map((service) => service.id)),
)
    ...
    <div class="employee-strip__list">
      ...
      >
        ...
        <div class="employee__info">
          ...
          <span v-if="integratedMode && employee.storeName" class="employee__store">
            {{ employee.storeName }}
          </span>
