...
const stores = computed(() =>
  (Array.isArray(props.overview?.stores) ? props.overview.stores : []).filter((store) =>
    shouldIncludeStore(store.storeId),
  ),
)

const waitingList = computed(() =>
  (Array.isArray(props.overview?.waitingList) ? props.overview.waitingList : []).filter((item) =>
    shouldIncludeStore(item.storeId),
  ),
)

const activeServices = computed(() =>
  (Array.isArray(props.overview?.activeServices) ? props.overview.activeServices : []).filter(
    (item) => shouldIncludeStore(item.storeId),
  ),
)
...
function formatServiceDuration(service) {
  ...
  return formatDuration(Math.max(0, finishedAt - Number(service?.serviceStartedAt || 0)), {
    roundUpPartialSecond: effectiveFinishedAt === 0,
  })
}

const pausedEmployees = computed(() =>
  (Array.isArray(props.overview?.pausedEmployees) ? props.overview.pausedEmployees : []).filter(
    (item) => shouldIncludeStore(item.storeId),
  ),
)

const availableConsultants = computed(() =>
  (Array.isArray(props.overview?.availableConsultants)
    ? props.overview.availableConsultants
    : []
  ).filter((item) => shouldIncludeStore(item.storeId)),
)
    ...
    <div class="operation-overview__summary">
      ...
      >
        ...
        <div class="operation-overview__summary-grid">
          <span>
            Fila:
            <strong>{{ store.waitingCount }}</strong>
          </span>
          <span>
            Atendimento:
          ...
          <span>
            Pausa:
            <strong>{{ store.pausedCount }}</strong>
          </span>
          <span>
            Disponiveis:
            <strong>{{ store.availableCount }}</strong>
          </span>
        </div>
      </article>
    </div>
        ...
        <div class="operation-overview__column-body">
          ...
          >
            ...
            <div class="operation-overview__person">
              <span class="queue-card__avatar" :style="{ '--avatar-accent': service.color }">
                {{ service.initials }}
              </span>
            ...
            <div class="operation-overview__details">
              ...
              <span>
                ...
                {{
                  formatClock(
                    Math.max(0, Number(service.serviceStartedAt || 0) - serverClockOffsetMs),
                  )
                }}
              </span>
          ...
          <div v-if="!activeServices.length" class="queue-empty">
            ...
            <span class="queue-empty__text">
              A visao integrada mostra todos os atendimentos das lojas acessiveis.
            </span>
          </div>
        </div>
      </section>
      ...
      <section class="operation-overview__column">
        ...
        <div class="operation-overview__column-body">
          ...
          <div v-if="!pausedEmployees.length" class="queue-empty">
            ...
            <span class="queue-empty__text">
              Quando alguem sair para tarefa, reuniao ou pausa, aparece aqui.
            </span>
          </div>
        </div>
      </section>
      ...
      <section class="operation-overview__column">
        ...
        <div class="operation-overview__column-body">
          ...
          <div v-if="!availableConsultants.length" class="queue-empty">
            ...
            <span class="queue-empty__text">
              Todos os consultores filtrados estao em fila, pausa ou atendimento.
            </span>
          </div>
        </div>
      </section>
