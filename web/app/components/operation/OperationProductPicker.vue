...
const remoteSearchMinChars = computed(() =>
  Math.max(1, Number(props.remoteSearchMinChars || 3) || 3),
)
...
const filteredOptions = computed(() => {
  ...
  return normalizedOptions.value.filter((item) => {
    ...
    return normalizeSearch(
      [item.label, item.meta, item.code, item.searchText].filter(Boolean).join(' '),
...
watch(
  () => normalizedSelectedItems.value.map((item) => item.id),
  (nextIds) => {
    ...
    if (
      !isSharedDetailMode.value &&
      itemDetailEditorId.value &&
      !nextIds.includes(itemDetailEditorId.value)
    ) {
      closeItemDetailEditor()
    }
  },
  ...
  <section class="finish-form__section operation-select-picker">
    ...
    <div ref="containerRef" class="product-pick">
      <div class="product-pick__inline-row">
        ...
        >
          <span class="product-pick__tag-content">
            <small v-if="isClosedMode && item.code" class="product-pick__closed-code">
              {{ item.code }}
            </small>
            <span class="product-pick__tag-label">{{ item.label }}</span>
          </span>
          <span v-if="isClosedMode && item.price > 0" class="product-pick__tag-price">
          ...
          <button
            ...
            :class="{
              'is-filled': hasItemDetail(item.id),
              'is-open': isItemDetailEditorOpen(item.id),
            }"
            :title="
              hasItemDetail(item.id)
                ? `Editar ${itemDetailLabel.toLowerCase()}`
                : `Adicionar ${itemDetailLabel.toLowerCase()}`
            "
            :aria-label="
              hasItemDetail(item.id)
                ? `Editar ${itemDetailLabel.toLowerCase()}`
          ...
          >
            <span class="material-icons-round">
              {{ hasItemDetail(item.id) ? 'edit_note' : 'note_add' }}
            </span>
          </button>
          <button
            type="button"
        ...
        <button
          ...
          :title="
            hasSharedDetail
              ? `Editar ${itemDetailLabel.toLowerCase()} compartilhada`
              : `Adicionar ${itemDetailLabel.toLowerCase()} compartilhada`
          "
          :aria-label="
            hasSharedDetail
              ? `Editar ${itemDetailLabel.toLowerCase()} compartilhada`
      ...
      <div
        v-if="
          enableItemDetails &&
          ((isSharedDetailMode && isSharedDetailEditorOpen) ||
            (!isSharedDetailMode && activeDetailItem))
        "
      ...
      <Teleport to="body">
        <template v-if="dropdownOpen">
          ...
          >
            ...
            <div
              v-if="allowCustom"
              class="product-pick__custom-form"
