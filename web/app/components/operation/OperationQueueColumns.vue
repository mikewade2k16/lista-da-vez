...
const cancelWindowSeconds = computed(
  () => Number(props.state.settings?.serviceCancelWindowSeconds || 30) || 30,
)
const modalConfig = computed(() => props.state.modalConfig || {})
const cancelReasonOptions = computed(() =>
  Array.isArray(props.state.cancelReasonOptions) ? props.state.cancelReasonOptions : [],
)
const stopReasonOptions = computed(() =>
...
const actionReasonOptions = computed(() => {
  const source =
    actionModal.value.action === 'cancel' ? cancelReasonOptions.value : stopReasonOptions.value
  return source
    .map((item) => ({
...
const actionReasonMode = computed(() => {
  ...
  const normalized =
    rawMode === 'select-with-other' || rawMode === 'select_other' || rawMode === 'select-other'
      ? 'select-with-other'
      : rawMode === 'select'
        ? 'select'
        : 'text'
...
const actionReasonPlaceholder = computed(() =>
  actionModal.value.action === 'cancel'
    ? String(
        modalConfig.value?.cancelReasonPlaceholder || 'Informe o motivo do cancelamento',
      ).trim()
    : String(modalConfig.value?.stopReasonPlaceholder || 'Informe o motivo da parada').trim(),
)

const actionOtherReasonLabel = computed(() =>
  actionModal.value.action === 'cancel'
...
const actionOtherReasonPlaceholder = computed(() =>
  actionModal.value.action === 'cancel'
...
const actionModalTitle = computed(() =>
  actionModal.value.action === 'cancel' ? 'Cancelar atendimento' : 'Parar atendimento',
)
...
function isSameSequentialGroup(targetService, candidate) {
  ...
  const siblingServiceIds = Array.isArray(candidate?.siblingServiceIds)
    ? candidate.siblingServiceIds
    : []
...
function resolveService(serviceOrId) {
  ...
  return (
    activeServices.value.find((item) => String(item?.serviceId || '').trim() === targetId) || null
  )
}
...
function resolveActionReason() {
  ...
  if (
    actionReasonMode.value === 'select-with-other' &&
    actionModal.value.reasonId === '__other__'
  ) {
    return String(actionModal.value.reasonText || '').trim()
  }

  return (
    actionReasonOptions.value.find((item) => item.value === actionModal.value.reasonId)?.label || ''
  )
}
...
async function openFinishModal(serviceOrId) {
  ...
  if (isWithinCancelWindow(service)) {
    const result = await operationsStore.serviceAction(service.serviceId, 'cancel', '', {
...
<template>
  <div class="workspace__intro" style="display: none">
    ...
    <p class="workspace__text">
      Toque em um funcionario abaixo para entrar na fila. O atendimento normal sai pelo botao do
      rodape e o fora da vez fica no card do consultor. Ao encerrar, abrimos o fechamento completo
      no modal.
    </p>
  </div>
    ...
    <section class="queue-column queue-column--waiting" data-testid="operation-waiting-column">
      ...
      >
        ...
        >
          {{
            isLimitReached
              ? `Limite de ${maxConcurrentServices} atendimentos ativos`
      ...
      <div class="queue-column__body queue-column__body--waiting">
          ...
          >
            ...
            <span class="queue-card__content">
              <span class="queue-card__headline">
                ...
                <span
                  v-if="props.integratedMode && person.storeName"
                  class="queue-card__store-badge"
                >
                  {{ person.storeName }}
                </span>
              </span>
              ...
              <span class="queue-card__note">
                {{ index === 0 ? 'Aguardando' : 'Aguardando na fila' }}
              </span>
            </span>
            <div class="queue-card__actions">
              <template v-if="(index === 0 && !props.integratedMode) || props.readOnly">
                ...
                <span v-else class="queue-card__badge">
        ...
        <div v-else class="queue-empty">
          ...
          <span class="queue-empty__text">
            {{
              props.readOnly
                ? 'Acompanhe por aqui quando a operacao da loja iniciar novos atendimentos.'
                : 'Use a barra de Consultores abaixo para colocar alguem na lista.'
            }}
          </span>
        </div>
      </div>
    ...
    <section class="queue-column queue-column--service" data-testid="operation-service-column">
      ...
      <div class="queue-column__body queue-column__body--service">
        ...
        <div v-else class="queue-empty">
          ...
          <span class="queue-empty__text">
            Quando iniciar um atendimento, o tempo passa a ser contado aqui.
          </span>
        </div>
      </div>
    </section>
  ...
  <Teleport to="body">
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        data-testid="operation-service-action-modal"
      >
        ...
        <div class="service-action-modal__body">
          ...
          <div v-if="showActionReasonField" class="service-action-modal__field">
            <label v-if="actionReasonMode !== 'text'" class="settings-card__text">
              {{ actionReasonLabel }}
            </label>
            <AppSelectField
              ...
              :options="[
                ...actionReasonOptions,
                ...(actionReasonMode === 'select-with-other'
            ...
            />
            <label
              v-if="shouldShowOtherReason"
              class="settings-field service-action-modal__textarea-wrap"
            >
              <span class="settings-card__text">
                {{ actionReasonMode === 'text' ? actionReasonLabel : actionOtherReasonLabel }}
              </span>
              <textarea
                v-model="actionModal.reasonText"
                class="service-action-modal__textarea"
                :placeholder="
                  actionReasonMode === 'text'
                    ? actionReasonPlaceholder
                    : actionOtherReasonPlaceholder
                "
                rows="4"
