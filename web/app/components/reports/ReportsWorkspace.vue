...
const multiStoreSummary = computed(
  () =>
    multiStoreOverview.value?.summary || {
      activeStores: 0,
      totalAttendances: 0,
      totalSoldValue: 0,
      totalQueue: 0,
...
const hourlyBars = computed(() => {
  ...
  return allHours.map((hour, index) => {
    ...
    const conversionHeight =
      conversions > 0 ? Math.max(2, (conversions / maxValue) * CHART_HEIGHT) : 0
...
const goalRows = computed(() => {
  const rosterWithAnyGoal = (props.state.roster || []).filter(
    (consultant) =>
  ...
  return rosterWithAnyGoal.map((consultant) => {
    const aggregate = report.value.chartData.consultantAgg.find(
    ...
    const conversionRate =
      aggregate.attendances > 0 ? (aggregate.conversions / aggregate.attendances) * 100 : 0
    const ticketAverage = aggregate.conversions > 0 ? soldValue / aggregate.conversions : 0

    const historyForConsultant = (props.state.serviceHistory || []).filter(
      (entry) => entry.personId === consultant.id,
    )
...
const teamGoalSummary = computed(() => {
  const totalGoal = (props.state.roster || []).reduce(
    (sum, item) => sum + Number(item.monthlyGoal || 0),
    0,
  )
  const totalSold = report.value.chartData.consultantAgg.reduce(
...
function toggleFilterValue(filterId, value) {
  const currentValues = Array.isArray(props.state.reportFilters?.[filterId])
    ? props.state.reportFilters[filterId]
    : []
  ...
  <section class="admin-panel" data-testid="reports-panel">
    ...
    <article v-else-if="reportsStore.pending && !reportsStore.ready" class="settings-card">
      <p class="settings-card__text">
        {{
          reportsStore.integratedScope
            ? 'Carregando relatorio geral...'
            : 'Carregando relatorios da loja ativa...'
        }}
      </p>
    </article>
    ...
    <template v-else-if="reportsStore.ready">
      <section
        v-if="reportsStore.integratedScope"
        class="metric-grid"
        data-testid="reports-multistore-summary"
      >
        <article class="metric-card">
          <span class="metric-card__label">Lojas ativas</span>
          <strong class="metric-card__value">{{ multiStoreSummary.activeStores }}</strong>
        </article>
        <article class="metric-card">
          <span class="metric-card__label">Atendimentos consolidados</span>
          <strong class="metric-card__value">{{ multiStoreSummary.totalAttendances }}</strong>
        </article>
        <article class="metric-card">
          <span class="metric-card__label">Vendas consolidadas</span>
          <strong class="metric-card__value">
            {{ formatCurrency(multiStoreSummary.totalSoldValue) }}
          </strong>
        </article>
        <article class="metric-card">
          <span class="metric-card__label">Fila atual total</span>
          <strong class="metric-card__value">{{ multiStoreSummary.totalQueue }}</strong>
        </article>
        <article class="metric-card">
          <span class="metric-card__label">Em atendimento agora</span>
          <strong class="metric-card__value">{{ multiStoreSummary.totalActiveServices }}</strong>
        </article>
        <article class="metric-card">
          <span class="metric-card__label">Score medio operacional</span>
          <strong class="metric-card__value">
            {{ Math.round(multiStoreSummary.averageHealthScore) }}
          </strong>
        </article>
      </section>
      ...
      <article
        v-if="reportsStore.integratedScope"
        class="insight-card insight-card--wide"
        data-testid="reports-multistore-table"
      >
      ...
      <section class="metric-grid" data-testid="reports-summary">
        ...
        <article class="metric-card">
          ...
          <strong class="metric-card__value">
            {{ formatPercent(report.metrics.queueJumpRate) }}
          </strong>
        </article>
        <article class="metric-card">
      ...
      <div class="report-chart-grid">
        ...
        <article class="insight-card">
          ...
          <span v-if="!report.chartData.hourlyData.length" class="insight-empty">
          ...
          <div v-else class="chart-hourly-wrap">
            <svg :viewBox="`0 0 ${CHART_WIDTH} ${CHART_HEIGHT + 18}`" width="100%">
              <g v-for="item in hourlyBars" :key="item.hour">
                ...
                />
                <rect
                  v-if="Number(item.conversionHeight) > 0"
                  :x="item.x"
                  :y="item.conversionY"
                  :width="item.width"
      ...
      <article v-if="!reportsStore.integratedScope" class="insight-card insight-card--wide">
        ...
        <span v-if="!goalRows.length" class="insight-empty">
          Nenhum consultor com meta definida. Configure metas em Configuracoes &gt; Consultores.
        </span>
        <template v-else>
          <div class="team-goal-summary">
            <div class="team-goal-summary__header">
              ...
              <span class="metric-card__text">
                {{ teamGoalSummary.totalSoldLabel }} de {{ teamGoalSummary.totalGoalLabel }}
              </span>
            </div>
            <div class="progress-bar progress-bar--team">
              <span
                class="progress-bar__fill"
                :style="{ '--progress': `${teamGoalSummary.progress.toFixed(1)}%` }"
              ></span>
            </div>
          </div>
          ...
          <div v-for="item in goalRows" :key="item.consultantId" class="consultant-goal-row">
            <span
              class="consultant-goal-row__avatar"
              :style="{ '--avatar-accent': item.consultantColor }"
            >
              {{ item.initials }}
            </span>
            <div class="consultant-goal-row__body">
              <div class="consultant-goal-row__header">
                ...
                <span
                  v-if="item.monthlyGoal && item.progress >= 100"
                  class="insight-tag insight-tag--success"
                >
                  Meta R$ atingida
                </span>
              </div>
              <template v-if="item.monthlyGoal">
                <div class="progress-bar">
                  <span
                    ...
                    :style="{
                      ...
                      background: `linear-gradient(90deg, ${item.consultantColor}88, ${item.consultantColor})`,
                    }"
                  ></span>
                </div>
                <div class="consultant-goal-row__footer">
                  ...
                  <span v-if="item.remaining > 0" class="metric-card__text">
                    Falta: {{ item.remainingLabel }}
                  </span>
                </div>
              </template>
              <div class="consultant-goal-indicators">
                <span
                  v-if="item.conversionGoal"
                  :class="[
                    'consultant-goal-badge',
                    item.conversionHit
                      ? 'consultant-goal-badge--hit'
                      : 'consultant-goal-badge--miss',
                  ]"
                >
                  Conv. {{ item.conversionRateLabel }} / meta {{ item.conversionGoalLabel }}
                </span>
                <span
                  v-if="item.avgTicketGoal"
                  :class="[
                    'consultant-goal-badge',
                    item.ticketHit ? 'consultant-goal-badge--hit' : 'consultant-goal-badge--miss',
                  ]"
                >
                ...
                <span
                  ...
                  :class="[
                    'consultant-goal-badge',
                    item.paHit ? 'consultant-goal-badge--hit' : 'consultant-goal-badge--miss',
                  ]"
                >
      ...
      <div class="report-dist-grid">
        <article class="insight-card">
          ...
          <span v-if="!report.chartData.topProductsClosed.length" class="insight-empty">
            Nenhum produto registrado.
          </span>
          <template v-else>
            <div
              v-for="item in report.chartData.topProductsClosed"
              :key="item.label"
              class="dist-bar-row"
            >
              ...
              <div class="dist-bar-row__track">
                <div
                  class="dist-bar-row__fill"
                  :style="{
                    width: `${((item.count / report.chartData.topProductsClosed[0].count) * 100).toFixed(1)}%`,
                  }"
                ></div>
              </div>
              <span class="dist-bar-row__count">{{ item.count }}</span>
        ...
        <article class="insight-card">
          ...
          <span v-if="!report.chartData.topVisitReasons.length" class="insight-empty">
            Nenhum motivo registrado.
          </span>
          <template v-else>
            <div
              v-for="item in report.chartData.topVisitReasons"
              :key="item.label"
              class="dist-bar-row"
            >
              <span class="dist-bar-row__label">{{ item.label }}</span>
              <div class="dist-bar-row__track">
                <div
                  class="dist-bar-row__fill"
                  :style="{
                    width: `${((item.count / report.chartData.topVisitReasons[0].count) * 100).toFixed(1)}%`,
                  }"
                ></div>
              </div>
              <span class="dist-bar-row__count">{{ item.count }}</span>
        ...
        <article class="insight-card">
          ...
          <span v-if="!report.chartData.topCustomerSources.length" class="insight-empty">
            Nenhuma origem registrada.
          </span>
          <template v-else>
            <div
              v-for="item in report.chartData.topCustomerSources"
              :key="item.label"
              class="dist-bar-row"
            >
              <span class="dist-bar-row__label">{{ item.label }}</span>
              <div class="dist-bar-row__track">
                <div
                  class="dist-bar-row__fill"
                  :style="{
                    width: `${((item.count / report.chartData.topCustomerSources[0].count) * 100).toFixed(1)}%`,
                  }"
                ></div>
              </div>
              <span class="dist-bar-row__count">{{ item.count }}</span>
