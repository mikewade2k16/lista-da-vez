...
export const ROADMAP_PHASES_PART2: RoadmapPhase[] = [
  ...
  },
  {
    ...
    tasks: [
      ...
      { id: "patch-local-realtime", label: "Eliminar refresh full e cascata de requests em cada alteracao de Tasks; reconciliar somente a entidade afetada", done: false, note: "DIVIDA DE PERFORMANCE CONFIRMADA EM 2026-07-28: uma unica acao pode repetir GET /boards e todas as paginas de /boards/{id}/tasks porque task.*/board.*/field.* ainda convergem para tasksWorkspace.refresh(). Alvo: mutation REST altera otimisticamente apenas a task/campo; evento WS carrega payload suficiente ou dispara GET individual da task e faz upsert/remove no Pinia; eventos de board/field atualizam somente metadados do board afetado; eventos duplicados dos canais account+board sao coalescidos por id+version. Nao fazer polling/refetch sem mudanca. Preservar criacao remota de task ausente, conflito de versao e fallback full somente como recuperacao. Aceite: uma edicao gera 1 mutation + no maximo 1 reconciliacao direcionada, sem waterfall de boards/paginacao." }
    ],
    verifiable: "Abrir modal em 2 abas → avatares mútuos visíveis com nick; focar campo → badge na outra aba; segundo user vê input :disabled e não consegue 'roubar' o lock; renomear card inline com 'palavra ' (espaço no final) funciona; editar em uma aba reflete na outra por reconciliação versionada e direcionada, sem refresh completo do workspace."
  },
  {
    id: "tasks-t8",
    code: "Tasks T8",
