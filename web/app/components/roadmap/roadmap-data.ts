...
export const ROADMAP_PHASES: RoadmapPhase[] = [
  ...
  // docs/FAXINA_CODIGO_2026-06-29.md.
  {
    ...
    tasks: [
      ...
      { id: "pre-f-rls", label: "[seguranca] RLS no Postgres (SEC-1): rede de isolamento final por baixo do resolveTenantScope (policy USING coluna = current_setting('app.account_id'))", done: false, note: "SUSPENSO (wave 4): fundação Go feita (Querier + middleware RLSConnGuard só em /v1/feedback + teste), MAS a migration piloto 0176 foi REMOVIDA — quebrou o boot (nome sem schema pegou a VIEW public.user_feedback, erro 42809) e o RLS seria inerte de qualquer jeito (app conecta como omni=SUPERUSER, que ignora RLS). 2 bloqueios: (1) role de app não-superuser sem BYPASSRLS + GRANTs + DATABASE_URL; (2) migration schema-qualificada nas tabelas base 'r' + security_invoker nas views. Detalhe: docs/RLS_PLAN.md + back/internal/modules/queue/feedback/AGENT.md." },
