...
export const ROADMAP_PHASES: RoadmapPhase[] = [
  ...
  },
  {
    ...
    tasks: [
      ...
      { id: "card-f12-overlay-barra-aviso", label: "Fase 12 — (1) Overlay configuravel nos fundos full-bleed (on/off, solido/linear, intensidade) via block.props; (2) Barra de aviso pelo Studio (config de SITE: ativar/desativar + texto + link), em SiteLayout.announcement, renderizada acima do PubHeader. 2 agentes paralelos, arquivos disjuntos.", done: false, note: "CODIGO ENTREGUE 2026-06-24 (2 agentes + orquestrador; api rebuildada, generate:prod re-rodado OK). Decisoes do dono: overlay default = solido escuro como hoje (paridade); barra de aviso nao dispensavel, link opcional. RESOLVIDO pelo orquestrador: havia uma PubAnnounceBar HARDCODED ('Hoje aberto das 10h as 19h') no public.vue; a barra do Studio a SUBSTITUIU (PubAnnounceBar removida, TAnnouncementBar e a unica) — entao apos deploy a barra so aparece quando ativada no Studio. Back PRECISOU: SiteLayout.Announcement no struct (model_layout.go), senao validateSiteLayout (unmarshal->marshal) dropava o campo no save. Agente A — Overlay (TAVOLA): block.props bgOverlay(none/solid/linear)/bgOverlayOpacity(leve/medio/forte)/bgOverlayDir; controles no StudioBgVideoPanel (painel vira 'Fundo'); SectionRenderer emite --bg-overlay (color-mix/linear-gradient, allowlist); .veil dos fundos full-bleed usa var(--bg-overlay, <default atual>) -> sem config = identico. Agente B — Barra de aviso (TAVOLA+back): SiteLayout.announcement{enabled,text,link?,linkLabel?}; useStudio announcement+updateAnnouncement (espelha updateTheme, persiste no doc via bridge); StudioAnnouncementPanel em pages/studio/index.vue ao lado do tema; useSiteLayout expoe announcement; public.vue renderiza TAnnouncementBar acima do PubHeader; back: DTO/persistencia do layout aceita announcement (jsonb opaco=nada; struct=add campo). AGENT.md (TAVOLA+back) atualizados pelo orquestrador. FALTA (dono): deploy do .output/public + ATIVAR a barra no Studio (a hardcoded saiu, sem ativar nao ha barra) + validacao visual (overlay nos heroes/cta/slider; barra de aviso)." },
      { id: "card-f11-fundo-video", label: "Fase 11 — Midia de fundo com VIDEO (imagem OU video) nos fundos full-bleed do TAVOLA (heroes + CTA fullbleed + slider banner). Config em block.props (sem migration, sem tocar no gerador, espelha imageRatio): bgVideo/bgVideoMobile/bgPoster/bgFit/bgFitMobile/bgPos/bgPosMobile. Video iPhone-first (autoplay+muted+playsinline+loop+preload, poster do 1o frame). 2 agentes paralelos.", done: false, note: "CODIGO ENTREGUE 2026-06-24 (2 agentes, integracao verificada: bridge upload-request/result + TMediaBg + media_storage). Decisoes do dono: superficies = heroes + CTA fullbleed + slider banner (NAO o Banner do painel, zero mudanca no model do restaurante); poster = 1o frame automatico (gerado NO CLIENTE via canvas no upload, subido como imagem pelo path existente — evita ffmpeg na imagem da api). Componente reusavel TMediaBg (img-ou-video iPhone-safe). StudioBlockEditor ganha painel 'Fundo (video)' gateado por familia (heroes/cta/sliders), igual ao painel imageRatio. SectionRenderer emite CSS vars --bg-fit/--bg-pos (+mobile). Agente A — fila-atendimento (back+bridge): media_storage.go aceita video/mp4|webm|quicktime + teto maior (5MB->~50MB video); handler multipart; receptor de upload do painel (lado Omni do bridge) aceita video; AGENT.md. Agente B — TAVOLA: TMediaBg + troca dos fundos hardcoded + painel Studio + useStudioBridge.studioUploadVideo + captura client-side do poster + CSS vars + mover/converter bg-mostarda.mov->public/video/bg-mostarda.mp4 (ffmpeg host). Teste de upload 11MB exige o teto novo. FIX dev/prod 2026-06-24: midia enviada pelo painel vem como /uploads/... (relativa a API Omni), entao TMediaBg absolutiza via resolveMediaUrl(url, apiBase) em utils/url.ts (so /uploads -> apiBase; /video do bundle e URLs absolutas ficam) — senao quebrava nos dois ambientes (resolvia no dominio do site). generate:prod RE-RODADO e validado (build OK, config prod embutida, /video/bg-mostarda.mp4 no bundle, sem localhost). FALTA (dono): rebuild api (libera UPLOAD de video) + deploy do .output/public + validacao no iPhone real (autoplay/loop/poster)." },
      { id: "card-f10-categoria-subtitulo-capa-banner", label: "Fase 10 — Título + subtítulo + capa + banner por categoria nos cards do TAVOLA. Contrato: description=subtítulo (já existe ponta-a-ponta, só expor no painel), imageUrl=capa (já existe), bannerUrl=coluna nova banner_url. Opção 'só título / título+subtítulo' = por categoria (subtítulo vazio = só título); sem toggle de seção (não mexe no registry gerado). 2 agentes paralelos por repo.", done: false, note: "EM ANDAMENTO 2026-06-23 (2 agentes paralelos). Decisões do dono: reusar description como subtítulo (zero schema novo p/ subtítulo), incluir capa(imageUrl)+banner(novo) nesta leva, controle por preenchimento (sem toggle). Agente A — fila-atendimento (back+painel): migration banner_url (idempotente) + Category/CategoryInput.BannerURL + store_catalog (columns/scan/create/update) + copyCategories + AGENT.md cardapio + painel domain/types.ts + CardapioSectionCategorias.vue (campo Subtítulo=description e Banner=upload; categoryBody manda description+bannerUrl). Agente B — TAVOLA (apps/web): types Category.bannerUrl + render do subtítulo(description) nos cards C01/C02/C04/C05/C06/C07, capa(imageUrl) já data-bound em C02/C05/C06; subtítulo só quando preenchido (v-if). ENTREGUE 2026-06-23: subtítulo + capa prontos ponta a ponta (back+painel+TAVOLA). GAP DO BANNER (avisar): bannerUrl é salvável no painel e vem no menu público, mas NENHUM card renderiza ainda — o único banner largo (C03 LibCategoriaBannerLargo) é editorial de uso único (f.image, sem dataBinding) e NÃO deve ser repurposado sem decisão; ligar banner por-categoria exige um layout data-bound novo/ajustado + tocar no def GERADO (.work/defs/categorias-*.json + gen-registry.cjs). Pendente: decidir onde o banner aparece (card-banner por categoria? header da página de categoria?) e wirar. FALTA (passos do dono): aplicar migration + docker compose up -d --build api + validação visual do subtítulo/capa no painel e no TAVOLA dev." },
      { id: "card-f13-plan", label: "Fase 13 — Tracking completo do TAVOLA + Analytics no painel (PLANO/SPECS): coleta ponta-a-ponta (acessos, páginas, produtos vistos, botões, quantidades, horários, tempo/dwell) trazida agregada p/ a API e um dashboard por estabelecimento. Workshop de design (4 specs + 2 críticas adversariais) reconciliado num doc canônico. Plano: docs/cardapio/PLANO_CARDAPIO_TRACKING_ANALYTICS.md", done: false, note: "PLANO/SPECS 2026-06-25. Escopo aprovado: completo reconciliado; LGPD por aviso+legítimo interesse (sem banner); execução paralela multi-agente após F1 (com ok explícito). Decisões canônicas: allowlist única de 36 eventos (+21 novos); dwell por evento page/product/section (agrega só final:true, sem double-count); chave de produto = product_slug validado; horários por created_at do servidor (occurredAt só ordena dentro do batch); endpoints /v1/cardapio/restaurants/{id}/analytics/*; batch obrigatório; tabela cardapio.sessions agregada na ingestão; dedupe por eventId; ip_hash com salt obrigatório no boot; anti-PII server-side (deny-list + menu_search só length/hasResults). NADA implementado." },
      { id: "card-f13-f1-ingestao", label: "F1 — Back ingestão (fila-atendimento): migration 0174 (colunas desnorm. em events + tabela sessions + índices), endpoint POST .../events/batch (≤50 eventos, ≤256KB, 202 {accepted,rejected}), allowlist +21, enriquecimento UA/referrer/ip_hash, anti-PII server-side, product_slug validado, dedupe por eventId, rate limit allowN por IP+slug, testes, AGENT.md. Rebuild api + env CARDAPIO_TELEMETRY_SALT.", done: true, note: "ENTREGUE + VALIDADO LOCAL 2026-06-25. go build/vet/test OK; migration 0174 aplicada (migration_up_ok); smoke do /events/batch ao vivo (accepted:3/rejected:1, PII removida, menu_search so length/hasResults, product_slug inexistente descartado, sessao agregada gravada). api rebuildada. FALTA (deploy): env CARDAPIO_TELEMETRY_SALT em prod." },
      { id: "card-f13-f2-analytics-api", label: "F2 — Back analytics API (fila-atendimento): model/service/store/http_analytics.go com 9 endpoints (overview, timeseries day|hour_of_day|weekday_hour, funnel, top-products, sources, devices, pages, dwell, clicks), 404 de pertencimento, lê sessions+events, bots excluídos, cardapio.view, testes. Depende do schema da F1. Rebuild api.", done: true, note: "ENTREGUE 2026-06-25 (agente). 9 endpoints sob /v1/cardapio/restaurants/{id}/analytics/*; go build/vet/test OK (12 testes); golangci-lint limpo; api rebuildada e rotas no ar (sem auth = 400 missing_account_id, gating do prefixo). FALTA: validacao autenticada no painel (browser)." },
      { id: "card-f13-f3-front-tavola", label: "F3 — Front TAVOLA: plugin telemetry.client.ts (singleton fila/batch/sendBeacon) + useTracking reescrito + identidade did/sid + dwell/scroll/heartbeat + data-block-id no SectionRenderer + handlers nomeados no CartDrawer + sessionId casando com orders + os 36 eventos. Depende da allowlist+batch da F1. Paralelo à F2.", done: false },
      { id: "card-f13-f4-dashboard", label: "F4 — Painel dashboard (fila-atendimento): aba 'Relatórios' no CardapioEditorWorkspace (gate:all, herda escopo) + ~13 componentes em components/cardapio/analytics/ + useCardapioAnalytics + tipos domain/cardapio/analytics.ts; reusa MetaAdsReportChart/AppDatePicker/CoreAsyncError; estados vazios honestos (sem mock). Depende dos endpoints da F2.", done: false, note: "CÓDIGO ENTREGUE (local) 2026-06-25. Tipos analytics.ts (9 DTOs + AnalyticsRange + helpers int/rate/duration). Store: analyticsRequest<T>(id,path,params) herda withScope+X-Account-Id (única camada de fetch). Composable useCardapioAnalytics(restaurantId,range): 1 bloco/endpoint (data/pending/error) + seletores locais (hoursGranularity/topProductsMetric/sourcesDimension/dwellDimension) + refresh global e por bloco; watch([id,range]) com debounce 250ms; Promise.all sem derrubar bloco. 14 componentes em analytics/: infra (Card/Chart/Toolbar/DonutList) + blocos Kpis(overview)/Trend(timeseries day)/Hours(hour_of_day|weekday_hour, toggle barras|heatmap)/Funnel(CSS)/TopProducts/Traffic(sources)/Devices(3 donuts)/Dwell/Pages/Clicks. Orquestrador CardapioSectionRelatorios.vue (grid 12 col, colapsa 1 col <=880px). Aba 'Relatorios' (gate:all) ligada no CardapioEditorWorkspace após Pedidos. Chart espelha MetaAdsReportChart (Apex via defineLazyComponent+ClientOnly, --primary/--success/--muted/--border em runtime, toolbar off, fundo transparente); KPIs espelham MetaAdsOverviewCard (skeleton). DESVIO: erro de bloco usa CoreErrorState (que CoreAsyncError reusa) p/ @retry real que re-chama só o refreshX — CoreAsyncError recarregaria a página (perderia o escopo do editor). Vazio honesto 'Sem dados neste período.', zero mock. NÃO rodado npm/git/docker. FALTA (dono): validação visual no dev contra os endpoints reais da F2." },
      { id: "card-f13-f5-lgpd-hardening", label: "F5 — LGPD/hardening: aviso de privacidade + opt-out no TAVOLA (legítimo interesse), job de retenção/pruning (90d linhas com device_id; rollup longo só anônimo), métrica server-side de rejected por nome, partição mensal de events se o volume provar. Banner de consentimento = débito documentado.", done: false, note: "CÓDIGO ENTREGUE + BACK VALIDADO 2026-06-25. (1) Métrica rejected: slog.Warn dos nomes fora da allowlist no batch (detecta drift de deploy fora de ordem, crítica B1). (2) Retenção/pruning: store.PruneTelemetry (DELETE parametrizado por make_interval em events.created_at + sessions.last_seen_at) + service.startRetentionLoop (goroutine diária, 1a poda ~5min pós-boot, parada no handle.Close via canal) + env CARDAPIO_TELEMETRY_RETENTION_DAYS (default 90, <=0 desliga); go build/vet/test OK, api rebuildada (poda no-op em dev pois nada tem 90d). (3) TAVOLA: PubPrivacyNotice (aviso dispensável no public.vue, legítimo interesse, sem banner bloqueante) + opt-out 'Não rastrear' (flag tavola:telemetry-optout que o plugin telemetry.client.ts lê → modo no-op). DÉBITO: partição mensal de events (volume-gated). FALTA: validação visual do aviso no dev." },
    ],
    blockers: [],
  ...
  // perfis custom por conta e overrides por usuario. Doc: docs/RBAC_ACESSO_MODELO.md.
  {
    ...
    tasks: [
      ...
      { id: "rbac-f1-modules", label: "Fase 1B: dar/remover módulo/página por usuário no drawer", done: true, note: "2026-06-26: aba Módulos = overrides core (core.user_permission_overrides) e aba Páginas = overrides do access legado (/v1/access/users/{id}/overrides, chaves workspace.*.view) — pois o gating de PÁGINA do menu resolve do access (auth.permissionKeys ← /v1/me/context), não do core. Registrado em docs/LEGADO.md item 5." },
      { id: "rbac-f1-agency", label: "Fase 1C: gerenciar papéis/módulos/páginas de usuário SÓ-organização (sem cliente), via escopo da conta-agência", done: true, note: "2026-06-26: AdminUserRolesPanel e AdminUserModulesPanel passam a incluir a conta-agência (is_agency) como escopo; backend já autoriza (CanAccessAccountRoles libera platform_admin/agency_owner). Páginas é per-usuário (access), independe de conta." },
      { id: "rbac-f2-map", label: "Fase 2: mapa módulo→páginas explícito + coerência usuário ⊆ account_modules no back (override só dev/admin) + mover gating de página para o core", done: false },
  ...
  // Template-core de modal (OmniEntityDrawer) + fim do flash de vazio em /tasks.
  // Doc: docs/frontend/MODAL_TEMPLATE.md.
  {
    id: "modal-template",
    code: "UX",
    title: "Modal-template único + loading de tasks",
    goal: "Todo modal usa uma casca única (OmniEntityDrawer) — ajustes num lugar só; e a página de tasks nunca mostra vazio antes dos dados (skeleton no boot e na troca de board).",
    status: "in_progress",
    estimateWeeks: "alguns dias",
    startedAt: "2026-06-26",
    tasks: [
      { id: "modal-core", label: "OmniEntityDrawer vira o template-core: header (fechar/expandir-toggle/popover de modo), resize no modo lado, toggle de fullscreen, slot #header-extra", done: true, note: "web/app/components/ui/OmniEntityDrawer.vue; docs/frontend/MODAL_TEMPLATE.md." },
      { id: "modal-tasks", label: "TasksTaskModal passa a montar sobre o OmniEntityDrawer (corpo/ações/rodapé nos slots); composable só espelha modo/largura via v-model", done: true, note: "Removido o USlideover próprio; Compartilhar/link/estrela no #header-extra (só no tasks)." },
      { id: "modal-users", label: "Modal de /manage/users herda o header do template (sem Compartilhar/link/estrela)", done: true, note: "AdminUserEditDrawer já usava OmniEntityDrawer; ganhou o header novo + resize + toggle de fullscreen." },
      { id: "tasks-skeleton", label: "Fim do flash de vazio em /tasks: skeleton no boot E na troca de board (board e tabela)", done: true, note: "activeBoardLoading em useTasksPageContext (pageBootstrapping || !loadedBoardIds.has(activeBoardId)); CoreSkeleton no TasksBoardView; :loading no TasksTableView." },
      { id: "modal-migrate-rest", label: "Migrar os demais modais (Account/Bio/Consultant/Ranking/Site/etc.) para o template-core, incremental", done: false }
    ],
    blockers: [],
    verifiable: "Abrir o modal de tasks e o de /manage/users: mesmo header; botão expandir entra E sai de fullscreen; resize funciona no modo lado; users sem Compartilhar/link/estrela. Recarregar /tasks e trocar de board: skeleton, sem flash de 'Sem tasks'. vue-tsc/eslint limpos."
  },
  // Programa de remoção de legado/mock do painel (auditoria ultracode 2026-06-26).
  // Inventário priorizado por risco; ver docs/LEGADO.md.
  {
    id: "legacy-cleanup",
    code: "LEGADO",
    title: "Remover legado/mock do painel (programa)",
    goal: "Zerar legado/mock: fonte única no banco real. Remoções seguras primeiro; access→core (gating de página, platform-wide) por último, em fases aditivas para não derrubar acesso.",
    status: "in_progress",
    estimateWeeks: "contínuo",
    startedAt: "2026-06-26",
    tasks: [
      { id: "lc-manual-sql", label: "Remover SQL manual obsoleto unify_users_view.sql (+ pasta manual/)", done: true, note: "Nunca foi rodado (migrator só roda migrations/). Deletado." },
      { id: "lc-auth-fallback", label: "Remover fallback legado de roles no auth (dead code) + flag AUTH_ROLES_SOURCE no-op", done: true, note: "resolveLegacyAuthRoleScope/findStoreIDs/resolveRole/legacyRoleProjection/authRolesSource apagados; resolveAuthRoleScope 100% core. go build + go test ./internal/modules/auth verdes; API rebuildada e healthy. Env var vira no-op (remover do compose no deploy-cleanup)." },
      { id: "lc-tasks-mock", label: "Remover mock de clientes de tasks (DEFAULT_CLIENT_OPTIONS/isMockClient/badge MOCK) — precisa backfill clientId int→uuid", done: false, note: "Risco médio: backfill DB + verificar clientes reais em core.accounts antes de tirar o fallback do front." },
      { id: "lc-seeds", label: "Neutralizar INSERTs em user_*_roles dropadas nas seeds 0002/0012/0015/0036", done: false, note: "Cuidado: seeds alimentam backfill 0133→core num DB fresh (papéis demo do dev)." },
      { id: "lc-mockqueue", label: "Remover mockQueueState/operationTemplates/DEFAULTs hardcoded após hidratação real verificada", done: false, note: "Alto risco: app-runtime.ts ainda usa mockQueueState como seed vivo; trocar por estado vazio + hidratação + fallback degradado (não mock)." },
      { id: "lc-localstorage", label: "Migrar UI state crítico (draft de comentário) de localStorage p/ banco", done: false, note: "Preferências cosméticas (tema/colunas) ficam client-side por design." },
      { id: "lc-access-core-additive", label: "access→core FASE 1 (aditiva): semear workspace.*.view/.edit no core + backfill role_permissions/overrides (sem trocar o gating)", done: true, note: "2026-06-26: 26 workspace.* em core.permissions (registry core/module.go, não-depreciadas) via migration 0175; backfill 359 role_permissions em 39 papéis + 2 overrides (paridade 1:1 com user_access_overrides). go build/test + API healthy + migration_up_ok. Zero blast radius (nada lê core workspace.* via has(); menu segue no access)." },
      { id: "lc-access-core-switch", label: "access→core FASE 2 (switch+remoção): auth.permissionKeys/gating lê do core (/v2/me/context); depois remover módulo access, /v1/access/*, /v1/me/context legado, /operacao/usuarios, ROLE_WORKSPACES", done: false, note: "platform-wide / risk alto: PRÉ-REQUISITO = verificar paridade por-usuário (core vs access) + canary; só então o switch atômico (front+back no mesmo deploy). NÃO fazer sem OK explícito. Ver LEGADO.md item 5 e backlog P1·L1 do panorama." },
    ],
    blockers: [],
  ...
  // docs/FAXINA_CODIGO_2026-06-29.md.
  {
    ...
    estimateWeeks: "wave 1 + wave 2 commitadas; faltam smokes no browser + deferidos (split useTasksPageContext, slugify) + pré-existentes",
    group: "multi-tenant",
    tasks: [
      ...
      { id: "pre-d-wave2-helpers", label: "[wave2] Consolidar helpers cross-módulo (firstNonEmpty 8x, normalizeStoreIDs cross-módulo, decodeStringSlice 3x, slugify 6x) num pacote utilitário único + testes", done: false, note: "FEITO wave 2 (commitado): pacote back/internal/platform/stringsx (FirstNonEmpty/NormalizeIDs/DecodeJSONStringSlice) + teste; ~18 call-sites migrados. PENDENTE: slugify (F-04) — 6 variantes geram slugs diferentes (NFD × sem-acento × só-lowercase); unificar muda o slug = decisão de produto." },
      { id: "pre-d-wave2-splits", label: "[wave2] Split de arquivos > 450 linhas: useTasksPageContext.ts (3063), AdminUsersWorkspace.vue (881), AdminRoleMatrixEditor.vue (633), admin_users_repository.go (592)", done: false, note: "FEITO wave 2: AdminUsersWorkspace.vue 881→491 (F-18), AdminRoleMatrixEditor.vue 633→492 (F-19), admin_users_repository.go 592→~420 (F-20) + struct RoleTemplate removida. PENDENTE: split do useTasksPageContext.ts 3063 (F-17) — adiado, fazer incremental com validação no browser." },
      { id: "pre-d-wave2-socket-inline-chat", label: "[wave2] Extrair socket WS compartilhado (useTasksRealtime/useTaskPresence), useInlineEditManager (6 managers) e composable de chat do feedback", done: true, note: "FEITO wave 2 (commitado): useRealtimeSocket.ts (socket WS compartilhado) + useInlineEditManager.ts (6 managers via inject) + useFeedbackChat.js (chat admin+user). Refactor puro, gates verdes (go build/vet/test + eslint 0 erros + vue-tsc sem erro novo). Falta smoke de realtime/inline-edit no browser." },
      // (e) Wave 3 — arrumar PRE-EXISTENTES (em andamento 2026-06-29). Modelos: opus high nas 2 difíceis, sonnet high no resto.
      { id: "pre-e-w3-vuetsc", label: "[wave3] Deixar o vue-tsc da baseline verde (sem any/@ts-ignore): tipar os {}/unknown em api-client.ts + runtime-remote* (opus), corrigir sourceValue/cast/typo prioritySet nos composables de tasks, e o tipo da prop empty/caption em OmniDataTable/TasksBoardView", done: false, note: "Wave 3: alvos corrigidos (api-client ApiRequestMethod, runtime-remote tipados, sourceValue/casts nos composables, prop creatable em OmniSelect destravando OmniDataTable/TasksBoardView), sem any/@ts-ignore. Total vue-tsc 348→340. PARCIAL: a baseline AINDA tem ~340 erros pré-existentes (apiRequest sem genérico->unknown, AppDatePicker date-lib) — 'vue-tsc 100% verde' é frente dedicada (centenas de erros). ADIADO a pedido (2026-06-29): avaliar SE vale e O QUE acarreta antes de mexer nos ~340 erros pré-existentes — NÃO tocar sem decisão (risco/retorno ainda incerto)." },
      { id: "pre-e-w3-n1", label: "[wave3] Eliminar o N+1 do MeAccounts/MeContext: batch ListEnabledModuleIDsForAccounts (WHERE account_id = ANY($1)) em vez de 1 query por conta — OPT-2/F-22", done: true, note: "FEITO wave 3 (revisão opus + build da imagem verde/healthy): batch ListEnabledModuleIDsForAccounts (WHERE account_id = ANY($1::uuid[])) no MeAccounts; MeContext (N=1) segue no singular; fallback p/ fakes de teste preservado. Validar no browser: switcher de contas com os módulos certos por conta." },
      { id: "pre-e-w3-compose", label: "[wave3] Remover o AUTH_ROLES_SOURCE morto dos docker-compose + .env*.example + marcar no LEGADO (F-08)", done: true, note: "FEITO wave 3: AUTH_ROLES_SOURCE removido dos docker-compose + .env*.example; auth/AGENT.md (linha ~80) e docs/LEGADO.md atualizados. Go não lia a flag em runtime desde a wave 1." },
      // (f) Frentes pesadas pós-revisão (em andamento 2026-06-29) — decididas com o usuário.
      { id: "pre-f-rls", label: "[seguranca · FUTURO/PARADO] RLS no Postgres (SEC-1): 2ª rede de isolamento multi-tenant (defesa em profundidade)", done: false, note: "PARADO p/ outro momento (decisão 2026-06-29). O QUE É: Row-Level Security, recurso nativo do Postgres — uma regra (policy) no banco que filtra automaticamente QUAIS LINHAS cada query enxerga, mesmo sem WHERE. POR QUÊ: hoje o isolamento entre clientes é 100% no código (filtro tenant_id no app, que FUNCIONA); o RLS seria a 2ª rede — se um dia uma query nova esquecer o filtro, o próprio BANCO barra o vazamento. STATUS: só IDEIA. A tentativa de fundação (wave 4) foi REMOVIDA do código em 2026-06-29 (era inerte sob superuser + a migration quebrava o boot; o código fica no histórico em cabf5b2 se precisar). A proteção real continua sendo o filtro de tenant no app. Quando refazer, será no SISTEMA INTEIRO, controlado. 2 BLOQUEIOS pra virar real: (1) a app conecta como omni=SUPERUSER, que o Postgres faz IGNORAR RLS — precisa de um role de app comum (não-superuser, sem BYPASSRLS) + GRANTs + DATABASE_URL; (2) os public.* viraram VIEWS (RLS não vai em view) — migration schema-qualificada nas tabelas base + security_invoker. PERFORMANCE (a AVALIAR antes de ligar): +1 predicado por query (pequeno COM índice em tenant_id, que a maioria das tabelas já tem, mas medir num endpoint de lista grande) + conn-por-request. Design completo: docs/RLS_PLAN.md." },
      { id: "pre-f-slugify", label: "[faxina] slugify canônico (F-04): unificar as 6 variantes numa regra única (NFD/sem-acento), back (Go) + front (TS) com o MESMO output", done: true, note: "2026-06-29: stringsx.Slugify (Go, NFD) + domain/utils/slugify.ts (TS, p{M}). bio/cardapio trocaram cópias locais; site.perolaSlug é outra lógica (crow-notion, usa '_') e não foi tocada. Slugs gravados no banco não são re-gerados." },
      { id: "pre-f-tasks-split-lazy", label: "[faxina/perf] Split do useTasksPageContext.ts (3063, F-17) + lazy-mount dos cards (F-24) — mesma área web/tasks", done: false, note: "Refactor de reatividade ALTO risco: incremental, preservando return/efeitos; EXIGE smoke no browser (board). Lazy-mount = montar editores pesados só no clique." },
      { id: "pre-f-projlean", label: "[perf] Projeção lean (OPT-4/F-26): listagens devolvem só os campos que a tela usa", done: false, note: "BEHAVIOR-AFFECTING (muda o shape da resposta) — conservador: só cortar campo PROVADAMENTE não usado pelo front, por endpoint; campo incerto fica. Senão quebra a tela silenciosamente." },
    ],
    blockers: [],
