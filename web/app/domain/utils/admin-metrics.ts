...
function extractClosedProductLabels(entry) {
  ...
  const fallback = String(
    entry?.productClosed || entry?.productSeen || entry?.productDetails || '',
  ).trim()
...
function resolveLiveStatusSnapshot({
  ...
  return {
    status: 'available',
    startedAt: currentStatus?.status === 'available' ? Number(currentStatus.startedAt || now) : now,
  }
}

export function buildConsultantStats({
  history,
  consultantId,
  monthlyGoal = 0,
  commissionRate = 0,
  ...
  const conversionRate = monthEntries.length
    ? (convertedEntries.length / monthEntries.length) * 100
    : 0
  const averageDurationMs = monthEntries.length
    ? monthEntries.reduce((sum, entry) => sum + Number(entry.durationMs || 0), 0) /
      monthEntries.length
    : 0
  const ticketAverage = convertedEntries.length ? soldValue / convertedEntries.length : 0
  const piecesFromConverted = convertedEntries.reduce((sum, entry) => {
    const closed = Array.isArray(entry.productsClosed) ? entry.productsClosed.length : 0
    return sum + closed
  }, 0)
  const totalPieces = Math.max(piecesFromConverted, convertedEntries.length)
  const paScore = convertedEntries.length ? totalPieces / convertedEntries.length : 0
...
export function buildRankingRows({ history, roster, scope = 'month', dateFrom = '', dateTo = '' }) {
  ...
  return roster
    .map((consultant) => {
      ...
      const piecesFromConverted = converted.reduce((sum, entry) => {
        const closed = Array.isArray(entry.productsClosed) ? entry.productsClosed.length : 0
        return sum + closed
      }, 0)
      const totalPieces = Math.max(piecesFromConverted, converted.length)
      const paScore = converted.length ? totalPieces / converted.length : 0

      const completeEntries = entries.filter((entry) => {
        const hasCustomer =
          Boolean(String(entry.customerName || '').trim()) ||
          Boolean(String(entry.customerPhone || '').trim())
        const hasProduct =
          (Array.isArray(entry.productsSeen) && entry.productsSeen.length > 0) ||
          Boolean(String(entry.productSeen || '').trim()) ||
      ...
      const avgDurationMs = entries.length
        ? entries.reduce((sum, e) => sum + Number(e.durationMs || 0), 0) / entries.length
        : 0
...
export function buildConsultantAlerts({ roster, history, settings }) {
  ...
  ;(roster || []).forEach((consultant) => {
    const monthEntries = (history || []).filter(
      (entry) =>
        entry.personId === consultant.id && getMonthStamp(entry.finishedAt) === currentMonth,
    )

    if (!monthEntries.length) return

    const converted = monthEntries.filter(
      (e) => e.finishOutcome === 'compra' || e.finishOutcome === 'reserva',
    )
    ...
    const ticketAverage = converted.length ? soldValue / converted.length : 0
    const totalPieces = monthEntries.reduce(
      (sum, e) => sum + (Array.isArray(e.productsClosed) ? e.productsClosed.length : 0),
      0,
    )
...
export function buildInsights({ history, visitReasonOptions = [], customerSourceOptions = [] }) {
  ...
  const convertedEntries = history.filter(
    (entry) => entry.finishOutcome === 'compra' || entry.finishOutcome === 'reserva',
  )
  const soldProducts = groupLabels(
    convertedEntries.flatMap((entry) => extractClosedProductLabels(entry)),
  )
  const requestedProducts = groupLabels(
    history.map((entry) => entry.productSeen || entry.productDetails),
  )
  const visitReasons = groupLabels(
    history.flatMap((entry) =>
      (entry.visitReasons || []).map((id) => visitReasonMap.get(id) || id),
    ),
  )
  const customerSources = groupLabels(
    history.flatMap((entry) =>
      (entry.customerSources || []).map((id) => customerSourceMap.get(id) || id),
    ),
  )
...
export function buildTimeIntelligence({
  ...
  const converted = history.filter(
    (entry) => entry.finishOutcome === 'compra' || entry.finishOutcome === 'reserva',
  )
  const queueJumpServices = history.filter((entry) => entry.startMode === 'queue-jump')
  const quickHighPotential = converted.filter(
    (entry) => Number(entry.durationMs || 0) <= fastThresholdMs,
  )
  const longLowSale = converted.filter(
    (entry) =>
      Number(entry.durationMs || 0) >= longThresholdMs &&
      Number(entry.saleAmount || 0) <= lowSaleThreshold,
  )
  const longNoSale = history.filter(
    (entry) =>
      entry.finishOutcome === 'nao-compra' && Number(entry.durationMs || 0) >= longThresholdMs,
  )
  const quickNoSale = history.filter(
    (entry) =>
      entry.finishOutcome === 'nao-compra' && Number(entry.durationMs || 0) <= fastThresholdMs,
  )
  const avgQueueWaitMs = history.length
    ? history.reduce((sum, entry) => sum + Number(entry.queueWaitMs || 0), 0) / history.length
    : 0
  const completedSessions = Array.isArray(consultantActivitySessions)
    ? consultantActivitySessions
    : []
...
export function buildOperationalIntelligence({
  ...
  const quickNoSaleRate = noSaleEntries.length
    ? (time.quickNoSaleCount / noSaleEntries.length) * 100
    : 0
  const longNoSaleRate = noSaleEntries.length
    ? (time.longNoSaleCount / noSaleEntries.length) * 100
    : 0
  const quickCloseRate = convertedEntries.length
    ? (time.quickHighPotentialCount / convertedEntries.length) * 100
    : 0
  const longLowSaleRate = convertedEntries.length
    ? (time.longLowSaleCount / convertedEntries.length) * 100
    : 0
  const queueToServiceRatio =
    time.consultantsInServiceMs > 0 ? time.consultantsInQueueMs / time.consultantsInServiceMs : 0
  const idleVsServiceRatio =
    time.totalsByStatus.service > 0
      ? time.totalsByStatus.available / time.totalsByStatus.service
      : 0
  const diagnosis = []
