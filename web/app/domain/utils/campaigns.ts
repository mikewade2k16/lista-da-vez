...
function calculateCampaignBonus(campaign, historyEntry) {
  ...
  const totalBonus =
    toNonNegativeNumber(campaign.bonusFixed) + saleAmount * toNonNegativeNumber(campaign.bonusRate)
...
export function normalizeCampaign(rawCampaign = {}) {
  ...
  const targetOutcome = VALID_OUTCOMES.has(merged.targetOutcome)
  ...
  const campaignType = VALID_CAMPAIGN_TYPES.has(merged.campaignType)
    ? merged.campaignType
...
function extractClosedProductCodes(historyEntry: LooseRecord) {
...
export function applyCampaignsToHistoryEntry(campaigns = [], historyEntry: LooseRecord = {}) {
  ...
  campaigns.forEach((rawCampaign) => {
    ...
    const matchedProductCodes = campaign.productCodes.filter((code) =>
      closedProductCodes.includes(code),
    )
    ...
    if (
      !matchesExistingCustomer(campaign.existingCustomerFilter, historyEntry.isExistingCustomer)
    ) {
      return
    }
  ...
  return {
    matches,
    totalBonus: Number(
      matches.reduce((sum, item) => sum + Number(item.bonusValue || 0), 0).toFixed(2),
    ),
  }
}
...
export function buildCampaignPerformance(campaigns = [], history = []) {
  ...
  campaigns.forEach((rawCampaign) => {
    ...
    const hitEntries = periodHistory.filter(
      (entry) =>
        Array.isArray(entry.campaignMatches) &&
        entry.campaignMatches.some((m) => m.campaignId === campaign.id),
    )
