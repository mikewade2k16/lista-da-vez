...
type BuildReportDataInput = {
  history?: LooseRecord[]
  roster?: LooseRecord[]
  visitReasonOptions?: LooseRecord[]
  customerSourceOptions?: LooseRecord[]
  filters?: LooseRecord
}
...
function normalizeFilterList(primaryValue, legacyValue) {
  const source =
    primaryValue !== undefined ? primaryValue : legacyValue !== undefined ? legacyValue : []
  ...
  return [
    ...new Set(
      normalized.map((item) => String(item || '').trim()).filter((item) => item && item !== 'all'),
    ),
  ]
}
...
function matchesReportFilters(entry, filters, consultantName) {
  ...
  if (
    !matchesSelectedValue(entry.isExistingCustomer ? 'yes' : 'no', filters.existingCustomerModes)
  ) {
    return false
  }
...
function buildReportRows(rows = [], visitReasonOptions = [], customerSourceOptions = []) {
  ...
  return (Array.isArray(rows) ? rows : []).map((row) => {
    const visitReasons = (row.visitReasons || [])
      .map((item) => visitReasonMap.get(item) || item)
      .filter(Boolean)
    const customerSources = (row.customerSources || [])
      .map((item) => customerSourceMap.get(item) || item)
      .filter(Boolean)
    ...
    return {
      ...
      completionLevel,
      completionLabel:
        COMPLETION_LEVEL_LABELS[completionLevel] || COMPLETION_LEVEL_LABELS.incomplete,
      completionRate: Number(row.completionRate || 0),
      campaignNamesLabel:
        Array.isArray(row.campaignNames) && row.campaignNames.length
...
export function buildReportRowsFromApi(
  rows = [],
  visitReasonOptions = [],
...
export function normalizeReportFilters(filters: LooseRecord = {}) {
  return {
    ...
    existingCustomerModes: normalizeFilterList(
      filters.existingCustomerModes,
      filters.existingCustomer,
    ),
...
export function buildReportData({
  ...
  const consultantMap = new Map(
    (roster || []).map((consultant) => [consultant.id, consultant.name]),
  )
  const visitReasonMap = buildLabelMap(visitReasonOptions)
  const customerSourceMap = buildLabelMap(customerSourceOptions)
  const filteredHistory = history
    .filter((entry) =>
      matchesReportFilters(
        entry,
        normalizedFilters,
        consultantMap.get(entry.personId) || entry.personName,
      ),
    )
    .sort((a, b) => Number(b.finishedAt || 0) - Number(a.finishedAt || 0))
  const rows = filteredHistory.map((entry) => {
    const visitReasons = (entry.visitReasons || [])
      .map((item) => visitReasonMap.get(item) || item)
      .filter(Boolean)
    const customerSources = (entry.customerSources || [])
      .map((item) => customerSourceMap.get(item) || item)
      .filter(Boolean)
  ...
  const completeCount = rows.filter(
    (row) => row.completionLevel === 'complete' || row.completionLevel === 'excellent',
  ).length
  ...
  filteredHistory.forEach((entry) => {
    ...
    if (isConversion) {
      hb.conversions += 1
      hb.saleAmount += amount
    }
    hourlyMap.set(hourKey, hb)

    const cId = entry.personId || ''
    const cb = consultantChartMap.get(cId) || {
      consultantId: cId,
      consultantName: consultantMap.get(cId) || entry.personName || '-',
      attendances: 0,
      conversions: 0,
      saleAmount: 0,
    }
  ...
  const consultantQuality = [...consultantQualityMap.values()]
    .map((item) => {
      const completeRate = item.totalAttendances
        ? (item.completeCount / item.totalAttendances) * 100
        : 0
      const excellentRate = item.totalAttendances
        ? (item.excellentCount / item.totalAttendances) * 100
        : 0
      const notesRate = item.totalAttendances ? (item.notesCount / item.totalAttendances) * 100 : 0
      const incompleteRate = item.totalAttendances
        ? (item.incompleteCount / item.totalAttendances) * 100
        : 0
  ...
  return {
    ...
    metrics: {
      ...
      averageQueueWaitLabel: formatDurationMinutes(
        rows.length ? totalQueueWaitMs / rows.length : 0,
      ),
...
export function buildReportDataFromApi({
  ...
  const normalizedFilters = normalizeReportFilters(
    results?.filters || overview?.filters || DEFAULT_REPORT_FILTERS,
  )
  ...
  return {
    ...
    chartData: {
      ...
      hourlyData: Array.isArray(overview?.chartData?.hourlyData)
        ? overview.chartData.hourlyData
        : [],
      consultantAgg: Array.isArray(overview?.chartData?.consultantAgg)
        ? overview.chartData.consultantAgg
        : [],
      topVisitReasons: Array.isArray(overview?.chartData?.topVisitReasons)
        ? overview.chartData.topVisitReasons
        : [],
      topCustomerSources: Array.isArray(overview?.chartData?.topCustomerSources)
        ? overview.chartData.topCustomerSources
        : [],
      topProductsClosed: Array.isArray(overview?.chartData?.topProductsClosed)
        ? overview.chartData.topProductsClosed
        : [],
    },
    quality: {
