  ...
  <AdminAuthShell :title="shellTitle" :description="shellDescription" card-width="30rem">
    <template v-if="step === 'request'">
      <form class="admin-auth-form" autocomplete="on" novalidate @submit.prevent="submitRequest">
        ...
        <Transition name="admin-auth-fade">
          <div
            v-if="requestError || auth.lastError"
            class="admin-auth-alert admin-auth-alert--error"
          >
        ...
        <div class="admin-auth-actions">
          <NuxtLink class="admin-auth-link" to="/auth/login">Voltar para login</NuxtLink>
        </div>
      </form>
    </template>
    ...
    <template v-else-if="step === 'confirm'">
      ...
      <form class="admin-auth-form" autocomplete="off" novalidate @submit.prevent="submitConfirm">
        ...
        <Transition name="admin-auth-fade">
          <div
            v-if="confirmError || auth.lastError"
            class="admin-auth-alert admin-auth-alert--error"
          >
    ...
    <template v-else>
      ...
      <NuxtLink class="admin-auth-submit" to="/auth/login">Ir para login</NuxtLink>
    </template>
  </AdminAuthShell>
</template>
