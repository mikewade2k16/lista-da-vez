  ...
  return {
    ...
    updateConsultantProfile(consultantId, patch) {
      ...
      const commissionRate = Math.max(
        0,
        Number(patch.commissionRate ?? existing.commissionRate) || 0,
      )
      const conversionGoal = Math.max(
        0,
        Math.min(100, Number(patch.conversionGoal ?? existing.conversionGoal) || 0),
      )
      ...
      updateState({
        ...state,
        roster: state.roster.map((consultant) =>
          consultant.id === consultantId ? nextConsultant : consultant,
        ),
        waitingList: state.waitingList.map((item) =>
          item.id === consultantId ? { ...item, ...nextConsultant } : item,
        ),
        activeServices: state.activeServices.map((item) =>
          item.id === consultantId ? { ...item, ...nextConsultant } : item,
        ),
      })
