  ...
  return {
    ...
    updatePauseReasonOption(optionId, label) {
      ...
      const duplicate = state.pauseReasonOptions.find(
        (item) =>
          item.id !== optionId &&
          normalizeText(item.label).toLowerCase() === normalized.toLowerCase(),
      )
    ...
    addCancelReasonOption(label) {
      const state = getState()
      const { item, items } = appendUniqueOption(
        ...
        productCatalog: state.productCatalog.map((product) =>
          product.id === productId
            ? {
                ...
                code: String((patch.code ?? product.code) || '')
                  .trim()
                  .toUpperCase(),
                category:
                  String((patch.category ?? product.category) || '').trim() || 'Sem categoria',
