...
export function createWorkspaceActions({ getState, updateState }) {
  return {
    ...
    updateStore(storeId, patch) {
      ...
      const updatedStore = {
        ...
        defaultTemplateId: String(
          patch?.defaultTemplateId ?? existingStore.defaultTemplateId ?? '',
        ).trim(),
        ...
        avgTicketGoal: Math.max(
          0,
          Number(patch?.avgTicketGoal ?? existingStore.avgTicketGoal ?? 0),
        ),
        conversionGoal: Math.max(
          0,
          Math.min(100, Number(patch?.conversionGoal ?? existingStore.conversionGoal ?? 0)),
        ),
    ...
    archiveStore(storeId) {
      ...
      const nextActiveStoreDescriptor =
        nextStores.find((store) => store.id === nextActiveStoreId) || null
