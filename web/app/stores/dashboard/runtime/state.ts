...
import {
  initializeConsultantStatuses,
  reconcileConsultantStatuses,
} from '~/stores/dashboard/runtime/status'
...
export function extractStoreScopedState(sourceState) {
  return {
    _operationSnapshotFetchedAt: Math.max(
      0,
      Number(sourceState._operationSnapshotFetchedAt || 0) || 0,
    ),
    selectedConsultantId: sourceState.selectedConsultantId,
    consultantSimulationAdditionalSales: Number(
      sourceState.consultantSimulationAdditionalSales || 0,
    ),
...
export function normalizeServiceHistoryList(
  rawHistory,
  fallbackStoreId,
  fallbackStoreName,
  timestamp,
...
export function normalizeStoreScopedState(
  rawScopedState,
  fallbackScopedState,
  storeDescriptor,
  timestamp,
  ...
  const selectedConsultantId = roster.some(
    (consultant) => consultant.id === rawScopedState?.selectedConsultantId,
  )
  ...
  const scopedState = {
    ...
    consultantSimulationAdditionalSales: Math.max(
      0,
      Number(
        rawScopedState?.consultantSimulationAdditionalSales ??
          fallback.consultantSimulationAdditionalSales ??
          0,
      ) || 0,
    ),
    ...
    consultantCurrentStatus:
      ...
      typeof rawScopedState.consultantCurrentStatus === 'object'
        ? rawScopedState.consultantCurrentStatus
        : {},
    pausedEmployees: Array.isArray(rawScopedState?.pausedEmployees)
      ? rawScopedState.pausedEmployees
      : [],
  ...
  return {
    _operationSnapshotFetchedAt: Math.max(
      0,
      Number(
        rawScopedState?._operationSnapshotFetchedAt || fallback?._operationSnapshotFetchedAt || 0,
      ) || 0,
    ),
    ...scopedState,
...
export function createEmptyState() {
  ...
  return {
    ...
    settings: {
      ...
      maxConcurrentServicesPerConsultant: Number(
        defaultTemplate?.settings?.maxConcurrentServicesPerConsultant || 1,
      ),
      ...
      serviceCancelWindowSeconds: Number(
        defaultTemplate?.settings?.serviceCancelWindowSeconds || 30,
      ),
...
export function buildRandomFinishModalDraft(state, service) {
  ...
  const finishFlowMode =
    String(state.modalConfig?.finishFlowMode || '').trim() === 'erp-reconciliation'
      ? 'erp-reconciliation'
      : 'legacy'
  const products = state.productCatalog.length
    ? state.productCatalog
    : [{ name: 'Produto teste', basePrice: 1000 }]
  ...
  const visitReasons = sampleItems(state.visitReasonOptions, visitReasonCount).map(
    (item) => item.id,
  )
  const customerSources = sampleItems(state.customerSourceOptions, sourceCount).map(
    (item) => item.id,
  )
  const queueJumpReasons = state.queueJumpReasonOptions.length
    ? state.queueJumpReasonOptions
    : DEFAULT_QUEUE_JUMP_REASON_OPTIONS
  const pauseReasons = state.pauseReasonOptions.length
    ? state.pauseReasonOptions
    : DEFAULT_PAUSE_REASON_OPTIONS
  const lossReasons = state.lossReasonOptions.length
    ? state.lossReasonOptions
    : DEFAULT_LOSS_REASON_OPTIONS
  const isLossReasonMultiple = state.modalConfig?.lossReasonSelectionMode === 'multiple'
  const lossReasonConfiguredDetailMode = ['off', 'shared', 'per-item'].includes(
  ...
  const lossReasonDetails =
    lossReasonConfiguredDetailMode === 'off'
      ...
      : lossReasonConfiguredDetailMode === 'per-item'
        ? Object.fromEntries(
            selectedLossReasons.map((item) => [item.id, `Perda por ${item.label.toLowerCase()}`]),
          )
        : Object.fromEntries(
            selectedLossReasons.map((item) => [item.id, 'Cliente ainda avaliando a compra.']),
          )
  const names = ['Ana', 'Carla', 'Bruna', 'Mariana', 'Paula', 'Julia', 'Erika']
  const professions = state.professionOptions.length
...
export function hydrateState(nextState: LooseRecord = {}) {
  const sourceState: LooseRecord = nextState && typeof nextState === 'object' ? nextState : {}
  ...
  const legacyScopedState = {
    ...
    activeServices:
      sourceState.activeServices || (sourceState.inService ? [sourceState.inService] : []),
  ...
  return {
    ...
    cancelReasonOptions: Array.isArray(sourceState.cancelReasonOptions)
      ? normalizeSimpleOptions(sourceState.cancelReasonOptions)
      : baseState.cancelReasonOptions,
    stopReasonOptions: Array.isArray(sourceState.stopReasonOptions)
