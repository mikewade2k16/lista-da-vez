...
export const useFeedbackStore = defineStore('feedback', () => {
  ...
  function sortFeedbacks(feedbacks: FeedbackItem[]) {
    return [...feedbacks].sort(
      (left, right) => getFeedbackActivityTime(right) - getFeedbackActivityTime(left),
    )
  }
  ...
  function upsertMessages(feedbackId: string, messages: FeedbackMessageItem[]) {
    ...
    messagesByFeedbackId.value = {
      ...messagesByFeedbackId.value,
      [feedbackId]: Array.from(byId.values()).sort(
  ...
  function getFeedbackById(feedbackId: string) {
    ...
    return (
      items.value.find((feedback) => feedback.id === normalizedId) ||
  ...
  function applyLocalReadState(feedbackId: string, readAt?: string) {
    ...
    if (currentReadAt) {
      ...
      if (
        Number.isFinite(nextReadTime) &&
        Number.isFinite(currentReadTime) &&
        nextReadTime < currentReadTime
      ) {
        return currentReadAt
      }
    }
  ...
  async function submitFeedback(input: CreateFeedbackInput) {
    try {
      ...
      const message = getApiErrorMessage(err, 'Erro ao enviar feedback. Tente novamente.')
      error.value = message
  ...
  async function fetchFeedbacks(filters?: { kind?: string; status?: string; since?: string }) {
    ...
    try {
      ...
      const message = getApiErrorMessage(err, 'Erro ao carregar feedbacks. Tente novamente.')
      error.value = message
      return { ok: false, message }
    } finally {
      if (shouldShowLoading) {
  ...
  async function fetchMyFeedbacks(filters?: { kind?: string; status?: string; since?: string }) {
    ...
    try {
      ...
      const message = getApiErrorMessage(err, 'Erro ao carregar seus chamados. Tente novamente.')
      error.value = message
      return { ok: false, message }
    } finally {
      if (shouldShowLoading) {
  ...
  async function updateFeedback(id: string, input: UpdateFeedbackInput) {
    try {
      ...
      const message = getApiErrorMessage(err, 'Erro ao atualizar feedback. Tente novamente.')
      error.value = message
      return { ok: false, message }
    } finally {
      loading.value = false
  ...
  async function sendMessage(feedbackId: string, input: CreateFeedbackMessageInput) {
    try {
      ...
      const message = getApiErrorMessage(err, 'Erro ao enviar resposta. Tente novamente.')
      error.value = message
      return { ok: false, message }
    } finally {
      loading.value = false
  ...
  async function markFeedbackAsRead(feedbackId: string) {
    ...
    const previousReadAt = String(
      currentFeedback?.user_last_read_at || currentFeedback?.created_at || '',
    ).trim()
    ...
    try {
      ...
      const message = getApiErrorMessage(err, 'Erro ao marcar chamado como lido. Tente novamente.')
