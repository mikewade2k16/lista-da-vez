...
type OperationFinishPayload = Record<string, unknown> & {
  serviceId: string
  outcome: string
}
...
export const useOperationsStore = defineStore('operations', () => {
  ...
  async function refreshOperationSnapshot(storeId, options: LooseRecord = {}) {
    syncStoredServerClockOffset()
  ...
  async function refreshOverview() {
    ...
    try {
      ...
      overviewError.value = getApiErrorMessage(
        error,
        'Nao foi possivel carregar a operacao integrada.',
      )
      throw error
    } finally {
  ...
  async function searchCatalogProducts(input: LooseRecord = {}) {
    syncStoredServerClockOffset()
  ...
  async function runCommand(path, body: LooseRecord = {}, options: LooseRecord = {}) {
    syncStoredServerClockOffset()
  ...
  return {
    ...
    startParallelService(personId, storeId = '') {
      return runCommand(
        '/v1/operations/services/parallel',
        { personId },
        {
          storeId,
          refreshOverview: Boolean(storeId),
    ...
    serviceAction(serviceId, action, reason = '', options: LooseRecord = {}) {
    ...
    async finishService(serviceId, closureData: LooseRecord = {}, options: LooseRecord = {}) {
      ...
      const serviceContext =
        options?.service && typeof options.service === 'object' ? options.service : null
      const activeService =
        serviceContext ||
        (runtime.state.activeServices || []).find((item) => item.serviceId === serviceId) ||
        null
      const targetStoreId =
        normalizeText(options?.storeId || activeService?.storeId) || runtime.state.activeStoreId
      const activeStore =
      ...
      const campaignSeed = activeService
        ? {
            ...
            storeName: normalizeText(
              options?.storeName || activeService?.storeName || activeStore?.name,
            ),
            ...
            durationMs: Math.max(
              0,
              Date.now() - Number(activeService.serviceStartedAt || Date.now()),
            ),
            ...
            skippedPeople: Array.isArray(activeService.skippedPeople)
              ? activeService.skippedPeople
              : [],
            skippedCount: Array.isArray(activeService.skippedPeople)
              ? activeService.skippedPeople.length
              : 0,
      ...
      const campaignMatches = normalizeCampaignMatches(
        Array.isArray(closureData?.campaignMatches)
          ? closureData.campaignMatches
          : campaignResult.matches,
      )
      const campaignBonusTotal = Math.max(
        0,
        Number(closureData?.campaignBonusTotal ?? campaignResult.totalBonus ?? 0) || 0,
      )
      const finishPayload: OperationFinishPayload = {
        serviceId,
        outcome,
      }

      if (closureData?.isWindowService) {
      ...
      if (closureData?.productsSeenNone) {
        finishPayload.productsSeenNone = true
      }

      if (closureData?.visitReasonsNotInformed) {
        finishPayload.visitReasonsNotInformed = true
      }

      if (closureData?.customerSourcesNotInformed) {
      ...
      if (closureData?.isExistingCustomer) {
      ...
      if (result.ok !== false) {
        const hasProfession =
          customerProfession &&
          (runtime.state.professionOptions || []).some(
            (option) => String(option?.label || '').trim() === customerProfession,
          )
