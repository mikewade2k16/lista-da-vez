...
import { normalizeCampaign } from "@core/utils/campaigns";
import { cloneValue } from "@core/utils/object";
import { DEFAULT_REPORT_FILTERS, normalizeReportFilters } from "@core/utils/reports";
...
function normalizeQueueState(rawState) {
  ...
  const stores =
    Array.isArray(rawState?.stores) && rawState.stores.length
      ? rawState.stores.map((store) => ({
          ...store,
          defaultTemplateId: String(store.defaultTemplateId || ""),
          monthlyGoal: Math.max(0, Number(store.monthlyGoal || 0)),
          weeklyGoal: Math.max(0, Number(store.weeklyGoal || 0)),
          avgTicketGoal: Math.max(0, Number(store.avgTicketGoal || 0)),
          conversionGoal: Math.max(0, Math.min(100, Number(store.conversionGoal || 0))),
          paGoal: Math.max(0, Number(store.paGoal || 0))
        }))
      : baseState.stores;
