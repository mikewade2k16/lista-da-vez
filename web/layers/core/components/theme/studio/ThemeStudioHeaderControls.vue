      ...
      <div class="space-y-3">
        ...
        <div class="flex flex-wrap gap-2">
          <UButton
            ...
            color="primary"
            variant="solid"
            @click="emit('apply-selected-theme')"
          />
          <UButton
            ...
            color="neutral"
            variant="outline"
            @click="emit('reset-selected-theme')"
          />
        </div>
      </div>
      ...
      <div class="w-full max-w-xl space-y-3">
        <div class="grid gap-2 sm:grid-cols-[1fr_auto_auto]">
          ...
          <UButton
            ...
            color="neutral"
            variant="outline"
            @click="emit('save-custom-theme-name')"
          />
          <UButton
            ...
            color="primary"
            variant="soft"
            @click="emit('create-custom-from-selected')"
          />
        </div>

        <div
          class="flex flex-wrap items-center justify-between gap-2 rounded-[var(--radius-sm)] border border-[rgb(var(--border))] bg-[rgb(var(--surface))] px-3 py-2"
        >
          <span class="text-sm text-[rgb(var(--text))]">Modo detalhado (tokens individuais)</span>
          <USwitch
            :model-value="props.showDetailedEditors"
            @update:model-value="onDetailedToggle"
          />
        </div>
      </div>
    </div>
