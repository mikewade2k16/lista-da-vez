...
import ThemeStudioDetailedGrid from '../components/theme/studio/ThemeStudioDetailedGrid.vue'
import ThemeStudioHeaderControls from '../components/theme/studio/ThemeStudioHeaderControls.vue'
import ThemeStudioSimplePanel from '../components/theme/studio/ThemeStudioSimplePanel.vue'
import { useThemeStudio } from '../composables/useThemeStudio'

definePageMeta({
  layout: 'dashboard',
  workspaceId: 'themes',
  pageLabel: 'Theme Studio',
})
...
<template>
  <section class="space-y-4">
    ...
    <ThemeStudioHeaderControls
      :theme-order="themeOrder"
      :theme-labels="themeLabels"
      :selected-theme="selectedTheme"
      :has-custom-theme="hasCustomTheme"
      :custom-theme-name-draft="customThemeNameDraft"
      :show-detailed-editors="showDetailedEditors"
      :search="search"
      @update:selected-theme="selectTheme"
      @apply-selected-theme="applySelectedTheme"
      @reset-selected-theme="resetSelectedThemeOverrides"
      @update:custom-theme-name-draft="setCustomThemeNameDraft"
      @save-custom-theme-name="saveCustomThemeName"
      @create-custom-from-selected="createCustomFromSelected"
      @update:show-detailed-editors="setShowDetailedEditors"
      @update:search="setSearch"
    />

    <ThemeStudioSimplePanel
      v-if="isSimpleMode"
      :font-items="fontItems"
      :font-value="variableValueMap['font-sans'] ?? ''"
      :radius-keys="radiusKeys"
      :radius-value-map="radiusValueMap"
      :shadow-color="variableValueMap['shadow-color'] ?? ''"
      :shadow-glow-color="variableValueMap['shadow-glow-color'] ?? ''"
      :accent-values="{
      ...
      :page-header-state="pageHeaderState"
      :header-text-link-value="headerTextLinkValue"
      @update-font="onFontChange"
      @update-radius="onRadiusChange($event.key, $event.value)"
      ...
      @update-header-brand-bg="onHeaderBackgroundChange"
      @update-header-panel-bg="onHeaderPanelBackgroundChange"
      @toggle-header-backgrounds="onHeaderBackgroundLinkChange"
      @update-header-brand-blur="onHeaderBrandBlurChange"
      @update-header-panel-blur="onHeaderPanelBlurChange"
      @toggle-header-blurs="onHeaderBlurLinkChange"
      @update-header-border="onHeaderDividerChange"
      @update-header-separator="onHeaderSeparatorChange"
      @toggle-header-dividers="onHeaderDividerLinkChange"
      @toggle-header-text-follow="onHeaderTextFollowChange"
      @update-header-text="onHeaderTextChange"
      @toggle-header-primary-actions="onHeaderActionPrimaryChange"
      @toggle-header-interactions="onHeaderInteractionLinkChange"
      @update-header-hover="onHeaderInteractionChange"
      @update-header-active="onHeaderActiveChange"
      @update-header-shell-shadow="onHeaderShellShadowChange"
      @update-header-fade-top="onHeaderFadeTopChange"
      @update-header-fade-bottom="onHeaderFadeBottomChange"
    ...
    <ThemeStudioDetailedGrid
      v-else
      :sections="groupedSections"
      :font-items="fontItems"
      :group-label-by="toGroupLabel"
      :variable-value-map="variableValueMap"
      :radius-value-map="radiusValueMap"
      :triplet-model-value-map="tripletModelValueMap"
      :radius-keys="radiusKeys"
      :shadow-color-keys="shadowColorKeys"
      @update-font="onFontChange"
      @update-radius="onRadiusChange($event.key, $event.value)"
