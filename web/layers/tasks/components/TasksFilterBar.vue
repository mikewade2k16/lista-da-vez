...
<template>
  <div class="tasks-toolbar">
    ...
    <div v-if="activeProject" class="tasks-toolbar__filters">
      <div v-if="activeProject.filters.search" class="tasks-toolbar__search-group">
        <div class="tasks-toolbar__search" :class="{ 'is-open': searchOpen || !!filters.search }">
          <UInput
            ...
            placeholder="Buscar por titulo, descricao e tags..."
            size="sm"
        ...
        <UButton
          class="tasks-toolbar__filter-btn"
          icon="i-lucide-search"
          :color="filters.search || searchOpen ? 'primary' : 'neutral'"
          :variant="filters.search || searchOpen ? 'soft' : 'ghost'"
          size="sm"
          title="Buscar"
          :data-active="!!filters.search"
          @click="toggleSearch"
        />
      </div>

      <UPopover
        v-if="activeProject.filters.responsible"
        v-model:open="responsibleOpen"
        :content="{ side: 'bottom', align: 'end' }"
      >
        <UButton
          class="tasks-toolbar__filter-btn"
          icon="i-lucide-user"
          :color="filters.responsible ? 'primary' : 'neutral'"
          :variant="filters.responsible ? 'soft' : 'ghost'"
          size="sm"
          title="Responsavel"
          :data-active="!!filters.responsible"
        />
        <template #content>
          <div class="tasks-toolbar__filter-popover">
            <OmniSelectMenuInput
              v-model:open="responsibleInnerOpen"
      ...
      <UPopover
        v-if="viewerUserType === 'admin' && activeProject.filters.client"
      ...
      >
        <UButton
          ...
          :color="filters.clientId ? 'primary' : 'neutral'"
          :variant="filters.clientId ? 'soft' : 'ghost'"
          size="sm"
          title="Cliente"
          :data-active="!!filters.clientId"
        />
        <template #content>
          <div class="tasks-toolbar__filter-popover">
            <OmniSelectMenuInput
              v-model:open="clientInnerOpen"
      ...
      <UPopover
        v-if="activeProject.filters.type"
      ...
      >
        <UButton
          ...
          :variant="filters.type ? 'soft' : 'ghost'"
          size="sm"
          title="Tipo"
      ...
      <UButton
        v-if="activeProject.filters.hideArchived"
        class="tasks-toolbar__filter-btn"
        icon="i-lucide-archive"
        :color="filters.hideArchived ? 'primary' : 'neutral'"
        :variant="filters.hideArchived ? 'soft' : 'ghost'"
        size="sm"
        :title="filters.hideArchived ? 'Mostrar arquivadas' : 'Ocultar arquivadas'"
    ...
    <div v-if="activeProject" class="tasks-toolbar__stats">
      <span
        class="tasks-toolbar__stat tasks-toolbar__stat--neutral"
        :title="`Total: ${projectTasks.length}`"
      >
        {{ projectTasks.length }}
      </span>
      <span
        class="tasks-toolbar__stat tasks-toolbar__stat--primary"
        :title="`Filtradas: ${filteredTasks.length}`"
      >
        {{ filteredTasks.length }}
      </span>
      <span
        class="tasks-toolbar__stat tasks-toolbar__stat--warning"
        :title="`Arquivadas: ${projectTasks.filter((t) => t.archived).length}`"
      >
        {{ projectTasks.filter((t) => t.archived).length }}
      </span>
    </div>

    <div v-if="activeProject" class="tasks-toolbar__view-seg" role="tablist">
      <button
        class="tasks-toolbar__view-btn"
    ...
    <UButton
      ...
      icon="i-lucide-plus"
      :label="viewMode === 'table' ? 'Nova linha' : 'Nova task'"
      color="primary"
      variant="soft"
      size="sm"
      ...
      <template #content>
        <div class="tasks-toolbar__menu">
          ...
          <button
            class="tasks-toolbar__menu-item"
            type="button"
            :disabled="!activeProject || boardGroupBy !== 'status'"
            @click="createColumn"
          >
          ...
          <button
            v-if="activeProject && boardView.hiddenColumnIds.length"
            class="tasks-toolbar__menu-item"
            type="button"
          ...
          <div v-if="activeProject" class="tasks-toolbar__menu-info">
            ...
            Agrupar:
            <strong>{{ fieldLabel(boardGroupBy) }}</strong>
          </div>
        </div>
      </template>
    </UPopover>
