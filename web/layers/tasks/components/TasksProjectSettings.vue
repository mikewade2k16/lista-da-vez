...
<template>
  <USlideover
    ...
    description="Defina base, colunas, filtros e campos do card."
    ...
    <template #body>
      <div class="tasks-page__settings space-y-4">
        <div class="tasks-page__settings-field space-y-1">
          ...
          <UInput
            v-model="projectSettingsDraft.name"
            class="tasks-page__settings-input"
        ...
        <div class="tasks-page__settings-field space-y-1">
          ...
          <UTextarea
            v-model="projectSettingsDraft.description"
            class="tasks-page__settings-input"
            :rows="2"
        ...
        <div class="tasks-page__settings-field space-y-1">
          ...
          <OmniSelectMenuInput
            v-model="projectSettingsDraft.statuses"
            :items="statusOptions"
            placeholder="Criar ou selecionar coluna"
            :multiple="true"
            :creatable="true"
            :searchable="true"
        ...
        <div class="tasks-page__settings-grid grid gap-4 md:grid-cols-2">
          <div class="tasks-page__settings-field space-y-1">
            ...
            <OmniSelectMenuInput
              v-model="projectSettingsDraft.boardGroupBy"
              :items="BOARD_GROUP_OPTIONS"
        ...
        >
          <p class="tasks-page__settings-group-title mb-2 text-sm font-semibold">
            Padroes de criacao
          </p>
        ...
        <div class="tasks-page__settings-grid grid gap-4 md:grid-cols-2">
          <div class="tasks-page__settings-field space-y-1">
            ...
            <OmniSelectMenuInput
              v-model="projectSettingsDraft.responsibles"
          ...
          <div class="tasks-page__settings-field space-y-1">
            ...
            <OmniSelectMenuInput
              v-model="projectSettingsDraft.types"
              :items="typeOptions"
        ...
        >
          ...
          <div class="tasks-page__settings-switch-grid grid gap-2 sm:grid-cols-2">
            ...
            >
              ...
              <USwitch
                v-model="
                  projectSettingsDraft.cardFields[item.key as keyof TaskProjectItem['cardFields']]
                "
                :disabled="item.key === 'client' && viewerUserType === 'client'"
              />
            </label>
        ...
        >
          <p class="tasks-page__settings-group-title mb-2 text-sm font-semibold">
            Campos visiveis no board
          </p>
          <div class="tasks-page__settings-switch-grid grid gap-2 sm:grid-cols-2">
            <label
              v-for="item in FIELD_DEFS.filter(
                (field) => field.key !== 'title' && field.key !== 'archived',
              )"
              :key="`board-${item.key}`"
              class="tasks-page__settings-switch-row flex items-center justify-between gap-3 rounded-[var(--radius-sm)] border border-[rgb(var(--border))] px-3 py-2"
            >
              ...
              <USwitch
                :model-value="
                  fieldSwitchValue(projectSettingsDraft.boardVisibleFieldKeys, item.key)
                "
          ...
          <div class="tasks-page__settings-switch-grid grid gap-2 sm:grid-cols-2">
            ...
            >
              ...
              <USwitch
                :model-value="
                  item.key === 'title' ||
                  fieldSwitchValue(projectSettingsDraft.tableVisibleFieldKeys, item.key)
                "
                :disabled="
                  item.key === 'title' || (item.key === 'clientId' && viewerUserType === 'client')
                "
                @update:model-value="
                  setFieldSwitch(
        ...
        >
          <p class="tasks-page__settings-group-title mb-2 text-sm font-semibold">
            Campos visiveis no modal
          </p>
          <div class="tasks-page__settings-switch-grid grid gap-2 sm:grid-cols-2">
            <label
              v-for="item in FIELD_DEFS.filter((field) => field.key !== 'title')"
              :key="`modal-${item.key}`"
              class="tasks-page__settings-switch-row flex items-center justify-between gap-3 rounded-[var(--radius-sm)] border border-[rgb(var(--border))] px-3 py-2"
            >
              <span class="text-sm">{{ item.label }}</span>
              <USwitch
                :model-value="
                  fieldSwitchValue(projectSettingsDraft.modalVisibleFieldKeys, item.key)
                "
                :disabled="item.key === 'clientId' && viewerUserType === 'client'"
                @update:model-value="
                  setFieldSwitch(
      ...
      <div class="tasks-page__settings-footer flex w-full items-center justify-between gap-2">
        ...
        <div class="tasks-page__settings-footer-actions flex items-center gap-2">
          <UButton
            ...
            color="neutral"
            variant="ghost"
            @click="projectSettingsOpen = false"
          />
          <UButton
            label="Salvar"
            color="primary"
