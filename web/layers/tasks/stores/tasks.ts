...
function defaultCardFieldsConfig(
  value?: Partial<TaskProjectCardFieldsConfig>,
): TaskProjectCardFieldsConfig {
  return {
...
function defaultProjectDefaults(
  value?: Partial<TaskProjectDefaultsConfig>,
): TaskProjectDefaultsConfig {
  return {
...
function normalizeViewConfig(
  view: Partial<OrchestratorView> | undefined,
  fallback: OrchestratorView,
): OrchestratorView {
  ...
  const visibleFieldKeys =
    Array.isArray(view?.visibleFieldKeys) && view.visibleFieldKeys.length > 0
      ? view.visibleFieldKeys.map((key) => normalizeFieldKey(key)).filter(Boolean)
      : [...fallback.visibleFieldKeys]
  const modalVisibleFieldKeys =
    Array.isArray(view?.modalVisibleFieldKeys) && view.modalVisibleFieldKeys.length > 0
...
function normalizeUiViews(views: ProjectUiMetadata['views']) {
  ...
  return views
    .map((view) =>
      normalizeViewConfig(view, view?.type === 'table' ? fallbacks[1]! : fallbacks[0]!),
    )
...
function stripHtml(value: unknown) {
  return stripHtmlToText(value, 5000)
}
  ...
  return normalizeViewConfig(
    {
      ...
      name: normalizeText(view.name, 120) || fallback.name,
      type,
      groupByFieldKey: normalizeFieldKey(
        config.groupByFieldKey || config.groupByFieldId || fallback.groupByFieldKey,
      ),
      visibleFieldKeys:
        Array.isArray(config.visibleFieldKeys) && config.visibleFieldKeys.length > 0
          ? config.visibleFieldKeys.map((key: unknown) => normalizeFieldKey(key)).filter(Boolean)
          : [...fallback.visibleFieldKeys],
      modalVisibleFieldKeys:
        Array.isArray(config.modalVisibleFieldKeys) && config.modalVisibleFieldKeys.length > 0
          ? config.modalVisibleFieldKeys
              .map((key: unknown) => normalizeFieldKey(key))
              .filter(Boolean)
          : [...fallback.modalVisibleFieldKeys],
      hiddenColumnIds: Array.isArray(config.hiddenColumnIds)
        ? config.hiddenColumnIds
            .map((columnId: unknown) => normalizeText(columnId, 80))
            .filter(Boolean)
        : [],
...
function mapBoardColumns(columns: BackendColumn[]) {
  ...
  return mapped.length > 0
    ? sortColumns(mapped)
    : DEFAULT_TASKS_COLUMNS.map((column) => ({ ...column }))
}
...
function mapBoardFields(
  fields: BackendField[],
  existingProject?: TaskProjectItem,
): OrchestratorField[] {
...
function currentUserLabel(currentUser?: Record<string, any> | null) {
  return compactUserLabel(currentUser, 120)
}

function backendUserLabel(
  user: BackendUserMini | null | undefined,
  ...
  return compactUserLabel(user, 120) || normalizeText(userID, 120)
}
...
function normalizeTaskVideos(value: unknown): TaskVideoItem[] {
...
function mapTaskToStoreItem(
  ...
  const status = normalizeText(
    mappedColumn?.label || task.status || project.statuses[0] || DEFAULT_TASKS_COLUMNS[0]!.label,
    120,
  )
  ...
  const responsibleLabel =
    normalizeText(resolvedTaskUi?.responsible, 120) ||
    backendUserLabel(task.responsible, currentUser) ||
    responsibleUserId
  ...
  return {
    ...
    prioritySet:
      typeof resolvedTaskUi?.prioritySet === 'boolean'
        ? resolvedTaskUi.prioritySet
        : normalizePriority(task.priority) !== 'media',
...
function buildProject(
  ...
  return {
    ...
    responsibles: uniqueStrings([
      ...(projectUi?.responsibles || []),
      ...(existingProject?.responsibles || []),
      ...boardTasks.map((task) => task.responsible),
    ]),
    types: uniqueStrings([
      ...(projectUi?.types || []),
      ...(existingProject?.types || []),
      ...boardTasks.map((task) => task.type),
    ]),
    fields: mapBoardFields(Array.isArray(board.fields) ? board.fields : [], existingProject),
    views,
    activeViewId:
      activeViewId && availableViewIds.has(activeViewId)
        ? activeViewId
        : views[0]?.id || 'view-board',
    ...
    updatedAt:
      normalizeText(board.updatedAt, 80) || existingProject?.updatedAt || new Date().toISOString(),
  }
}
...
export const useTasksStore = defineStore('tasks', () => {
  ...
  async function initialize(options: { allowAutoCreate?: boolean } = {}) {
    ...
    try {
      ...
      await refresh()
      let normalizedLegacyColumns = false
  ...
  async function normalizeLegacyDefaultProject(project: TaskProjectItem) {
    ...
    let changed = false
    for (let index = 0; index < project.columns.length; index += 1) {
      const template = DEFAULT_TASKS_COLUMNS[index]
      if (!template) {
        continue
      }
      const currentProject = projects.value.find((item) => item.id === project.id) || project
      const currentColumn = currentProject.columns[index]
      if (!currentColumn) {
        continue
      }
      if (
        currentColumn.label !== template.label ||
        normalizeColumnColor(currentColumn.color) !== template.color ||
        currentColumn.order !== template.order
      ) {
        await updateColumn(project.id, currentColumn.id, {
    ...
    for (let index = project.columns.length; index < DEFAULT_TASKS_COLUMNS.length; index += 1) {
      const template = DEFAULT_TASKS_COLUMNS[index]!
      const created = await createColumn(project.id, template.label)
      if (!created) {
        continue
      }
      await updateColumn(project.id, created.id, {
        label: template.label,
        color: template.color,
        order: template.order,
      })
  ...
  async function normalizeLegacyDefaultProjects() {
    let changed = false
    for (const project of projects.value) {
      changed = (await normalizeLegacyDefaultProject(project)) || changed
    }
    return changed
  }
  ...
  async function createProject(rawName: unknown) {
    ...
    try {
      ...
      errorMessage.value = getApiErrorMessage(error, 'Nao foi possivel criar o projeto.')
      return null
    }
  }
  ...
  async function saveProjectSettings(
    ...
    nextProject.columns = payload.columns
      ? sortColumns(payload.columns.map((column) => ({ ...column })))
      : nextProject.columns
    nextProject.statuses = nextProject.columns.map((column) => column.label)
    nextProject.activeViewId =
      normalizeText(payload.activeViewId || nextProject.activeViewId, 80) ||
      nextProject.views[0]?.id ||
      'view-board'
    ...
    const list = tasks.value
      .filter(
        (task) =>
          ...
          normalizeKey(task.status) === normalizeKey(status) &&
  ...
  function resolveResponsibleUserId(
    currentTask: TasksStoreTaskItem | undefined,
    nextResponsible: unknown,
  ) {
    ...
    if (
      currentTask?.responsibleUserId &&
      normalizeText(currentTask.responsible, 120) === normalized
    ) {
  ...
  async function createTask(
    payload: Partial<Omit<TasksStoreTaskItem, 'id' | 'createdAt' | 'updatedAt'>> = {},
  ) {
    const projectId =
      normalizeText(payload.projectId, 80) || activeProjectId.value || projects.value[0]?.id || ''
    const project = projects.value.find((item) => item.id === projectId)
    if (!project) {
      return null
    }
    const status =
      normalizeText(payload.status, 120) || project.statuses[0] || DEFAULT_TASKS_COLUMNS[0]!.label
    ...
    const refreshedProject = buildProject(
      ...
      project,
      tasks.value.filter((item) => item.projectId === project.id),
      uiMetadata.value.projects[project.id],
    )
    replaceProject({
      ...refreshedProject,
      filters: project.filters,
  ...
  async function updateTask(
    taskId: string,
    patch: Partial<Omit<TasksStoreTaskItem, 'id' | 'projectId' | 'createdAt'>>,
  ) {
    ...
    const nextStatus = Object.prototype.hasOwnProperty.call(patch, 'status')
      ...
      : currentTask.status
    const nextColumn = nextStatus
      ? findColumnByStatus(project, nextStatus)
    ...
    if (Object.prototype.hasOwnProperty.call(patch, 'title'))
      optimisticTask.title = clampText(patch.title, 220)
    if (
      Object.prototype.hasOwnProperty.call(patch, 'contentHtml') ||
      Object.prototype.hasOwnProperty.call(patch, 'description')
    ) {
      const contentHtml = sanitizeTaskContentHtml(patch.contentHtml || patch.description)
    ...
    if (Object.prototype.hasOwnProperty.call(patch, 'prioritySet'))
      optimisticTask.prioritySet = Boolean((patch as TasksStoreTaskItem).prioritySet)
    if (Object.prototype.hasOwnProperty.call(patch, 'dueDate'))
      optimisticTask.dueDate = toDateOnly(patch.dueDate)
    if (Object.prototype.hasOwnProperty.call(patch, 'dueEndDate'))
      optimisticTask.dueEndDate = toDateOnly((patch as TasksStoreTaskItem).dueEndDate)
    if (Object.prototype.hasOwnProperty.call(patch, 'archived'))
      optimisticTask.archived = Boolean(patch.archived)
    if (Object.prototype.hasOwnProperty.call(patch, 'order'))
      optimisticTask.order = Number(patch.order || 0)
    if (Object.prototype.hasOwnProperty.call(patch, 'responsible'))
      optimisticTask.responsible = normalizeText(patch.responsible, 120)
    if (Object.prototype.hasOwnProperty.call(patch, 'involved')) {
      optimisticTask.involved = Array.isArray(patch.involved)
    ...
    const requestBody = {
      ...
      dueDate: Object.prototype.hasOwnProperty.call(patch, 'dueDate')
        ? toOptionalDateTime(patch.dueDate) || null
        : undefined,
    ...
    if (
      localTask &&
      localTask.title !== localTask.title.trim() &&
  ...
  async function moveTaskToStatus(taskId: string, status: string, targetIndex?: number) {
    ...
    const mapped = mapTaskToStoreItem(
      { ...task, status },
      project,
      uiMetadata.value.tasks[task.id],
      auth.user,
    )
    replaceTask(mapped)
    return mapped
    ...
    const nextProject = {
      ...
      columns: sortColumns([
        ...project.columns,
        {
          ...
          color: normalizeColumnColor(column.color),
          order:
            Number(column.sortOrder || (project.columns.length + 1) * 100) ||
            (project.columns.length + 1) * 100,
        },
      ]),
    }
    ...
    const response = await request(`/v1/tasks/columns/${encodeURIComponent(currentColumn.id)}`, {
      ...
      body: {
        label: Object.prototype.hasOwnProperty.call(patch, 'label')
          ? normalizeText(patch.label, 120)
          : undefined,
        color: Object.prototype.hasOwnProperty.call(patch, 'color')
          ? normalizeColumnColor(patch.color)
          : undefined,
        sortOrder: Object.prototype.hasOwnProperty.call(patch, 'order')
          ? Number(patch.order || 0)
          : undefined,
  ...
  async function deleteColumn(projectId: string, columnId: string, fallbackColumnId?: string) {
    ...
    const remapToColumnId =
      normalizeText(fallbackColumnId, 80) ||
      project.columns.find((item) => item.id !== currentColumn.id)?.id ||
      ''
    ...
    if (remapToColumnId) {
      ...
      tasks.value = tasks.value.map((task) =>
        task.projectId === project.id && task.columnId === currentColumn.id
          ? {
              ...task,
              columnId: remapToColumnId,
              status: fallbackColumn?.label || task.status,
              updatedAt: new Date().toISOString(),
            }
          : task,
      )
  ...
  async function moveColumn(projectId: string, columnId: string, targetIndex: number) {
    const project = projects.value.find((item) => item.id === normalizeText(projectId, 80))
    const currentIndex =
      project?.columns.findIndex((item) => item.id === normalizeText(columnId, 80)) ?? -1
    ...
    await Promise.all(
      nextColumns.map((column) =>
        request(`/v1/tasks/columns/${encodeURIComponent(column.id)}`, {
          method: 'PATCH',
